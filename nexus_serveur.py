...
================================================================================
  ...
  *** SUPPRESSION DÉFINITIVE (tombstones) ***
   Quand on supprime un compte (panel web ou écraseur), on pose une "pierre
   tombale" (db["deleted"][nom] = heure). La synchro (/admin/merge) refuse
   ensuite de recréer un compte tant que sa tombale est plus récente que la
   copie entrante -> plus de résurrection quand un serveur local se synchronise.
...
def load_db():
    try:
        ...
        db = {"users": {}}
    db.setdefault("users", {})
    db.setdefault("deleted", {})   # pierres tombales : nom -> heure de suppression
    return db
...
def tombstone(db, name):
    """Pose une pierre tombale et retire le compte. Suppression définitive."""
    db.setdefault("deleted", {})[name] = now_iso()
    db["users"].pop(name, None)


def clear_tombstone(db, name):
    """Lève la pierre tombale (compte (re)créé volontairement)."""
    db.setdefault("deleted", {}).pop(name, None)
...
async function supprimer(name) {
  if (!confirm("Supprimer DÉFINITIVEMENT « "+name+" » ? (il ne pourra plus revenir par synchro)")) return;
...
def register():
    ...
    with _lock:
        ...
        clear_tombstone(db, u)          # (re)création volontaire -> on lève la tombale
        save_db(db)
...
@app.post("/admin/purge")     # alias : même effet (utilisé par l'écraseur)
def admin_delete():
    d = request.get_json(force=True, silent=True) or {}
    with _lock:
        db = load_db()
        ...
        if d.get("target") in db["users"]:
            tombstone(db, d["target"])      # <<< suppression DÉFINITIVE (pierre tombale)
            save_db(db)
            return jsonify(ok=True)
    return jsonify(ok=False, error="introuvable")
...
def admin_purge_all():
    """Efface DÉFINITIVEMENT tous les comptes (garde les admins par sécurité)."""
    d = request.get_json(force=True, silent=True) or {}
    with _lock:
        db = load_db()
        if not admin_ok(d, db):
            return jsonify(ok=False, error="accès refusé")
        keep_admins = bool(d.get("keep_admins", True))
        for name in list(db["users"].keys()):
            if keep_admins and db["users"][name].get("role") == "admin":
                continue
            tombstone(db, name)
        save_db(db)
    return jsonify(ok=True)
...
def admin_rename():
    ...
    with _lock:
        ...
        db["users"][new]["updated"] = now_iso()
        clear_tombstone(db, new)     # le nouveau nom est vivant
        tombstone(db, t)             # l'ancien nom ne doit pas ressusciter
        # tombstone() a re-supprimé de users, mais le compte est sous 'new' -> OK
        save_db(db)
    return jsonify(ok=True)
...
def admin_nickname():
    ...
    with _lock:
        ...
        db["users"][d["target"]]["updated"] = now_iso()
        save_db(db)
    return jsonify(ok=True)
...
def admin_create():
    ...
    with _lock:
        ...
        clear_tombstone(db, u)          # création volontaire -> on lève la tombale
        save_db(db)
...
def admin_merge():
    """Fusionne une base entrante en respectant les pierres tombales.

    - union des comptes, on garde le plus récent (updated) ;
    - MAIS un compte dont la tombale est >= sa version entrante N'EST PAS recréé
      (fin des résurrections après suppression) ;
    - les tombales sont fusionnées entre serveurs (on garde la plus récente) et
      appliquées aux comptes locaux périmés ;
    - forum fusionné (par heure), extensions : on garde la plus récente."""
    d = request.get_json(force=True, silent=True) or {}
    ...
    with _lock:
        ...
        deleted = db.setdefault("deleted", {})

        # 1) union des pierres tombales (on garde l'heure la plus récente)
        for name, t in (incoming.get("deleted", {}) or {}).items():
            if name not in deleted or (t or "") > deleted[name]:
                deleted[name] = t

        # 2) applique les tombales aux comptes déjà présents (purge locale)
        for name in list(db["users"].keys()):
            t = deleted.get(name)
            if t and t >= db["users"][name].get("updated", ""):
                del db["users"][name]

        # 3) fusionne les comptes entrants SANS ressusciter un compte supprimé
        for name, u in (incoming.get("users", {}) or {}).items():
            t = deleted.get(name)
            if t and t >= u.get("updated", ""):
                continue   # supprimé après cette version -> on ignore
            cur = db["users"].get(name)
            if not cur or u.get("updated", "") > cur.get("updated", ""):
                db["users"][name] = u
                # version plus récente que la tombale -> compte revenu volontairement
                if name in deleted and u.get("updated", "") > deleted[name]:
                    del deleted[name]
