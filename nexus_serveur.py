...
# ==============================================================================
# ONGLET PREVISION — Vue prevision du prix NXC jusqu'a 30 jours
# Ce code est hors de la string NXC_PANEL_HTML pour etre lisible et maintenable
# ==============================================================================

_PREVISION_VIEW = (
    '<div class="view" id="view-prevision">'
    '<div class="card" style="margin-bottom:12px">'
    '<div class="ct" style="color:#a06bff;font-size:10px;'
    'letter-spacing:3px;margin-bottom:14px">\U0001f52e PREVISION NXC</div>'
    '<div style="font-size:11px;color:var(--muted);margin-bottom:14px;line-height:1.6">'
    'Estimation basee sur le prix actuel, le biais directionnel et la cible MR. '
    'Fourchette = confiance 90 pct.'
    '</div>'
    '<div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">'
    '<div style="flex:1;min-width:80px;background:rgba(0,229,255,.06);'
    ...
    '<div id="pv-current" style="font-size:17px;font-weight:800;'
    'color:var(--cyan);font-family:monospace">-</div>'
    '</div>'
    '<div style="flex:1;min-width:80px;background:rgba(160,107,255,.06);'
    'border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center">'
    '<div style="font-size:10px;color:var(--muted);margin-bottom:4px">TENDANCE</div>'
    '<div id="pv-trend" style="font-size:15px;font-weight:800">-</div>'
    '</div>'
    '<div style="flex:1;min-width:80px;background:rgba(0,255,157,.06);'
    'border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center">'
    '<div style="font-size:10px;color:var(--muted);margin-bottom:4px">CIBLE MR</div>'
    '<div id="pv-target" style="font-size:17px;font-weight:800;'
    'color:var(--green);font-family:monospace">-</div>'
    '</div>'
    '</div>'
    '<table id="pv-table" style="width:100%;border-collapse:collapse;font-size:12px">'
    '<thead>'
    '<tr style="border-bottom:1px solid var(--border)">'
    '<th style="text-align:left;padding:8px 6px;color:var(--muted);'
    'font-size:10px;font-weight:700;letter-spacing:1px">HORIZON</th>'
    '<th style="text-align:right;padding:8px 6px;color:var(--muted);'
    'font-size:10px;font-weight:700;letter-spacing:1px">ESTIME</th>'
    '<th style="text-align:right;padding:8px 6px;color:var(--red);'
    'font-size:10px;font-weight:700;letter-spacing:1px">MIN P10</th>'
    '<th style="text-align:right;padding:8px 6px;color:var(--green);'
    'font-size:10px;font-weight:700;letter-spacing:1px">MAX P90</th>'
    '<th style="text-align:center;padding:8px 6px;color:var(--muted);'
    'font-size:10px;font-weight:700;letter-spacing:1px">VAR.</th>'
    '</tr>'
    '</thead>'
    '<tbody id="pv-body">'
    '<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--muted)">'
    'Cliquez Recalculer pour lancer la prevision...</td></tr>'
    '</tbody>'
    '</table>'
    '<button class="btn full" onclick="calcPrev()" style="margin-top:14px">'
    '\U0001f504 Recalculer'
    '</button>'
    '<div style="margin-top:10px;font-size:10px;color:var(--muted);'
    'text-align:center;line-height:1.5">'
    'Prevision indicative — le marche NXC est stochastique par nature.'
    '</div>'
    '</div>'
    '<div class="card">'
    '<div class="ct" style="margin-bottom:12px;font-size:10px;letter-spacing:2px">'
    '\U0001f4c8 COURBE PREVISIONNELLE'
    '</div>'
    '<svg id="pv-svg" viewBox="0 0 320 110" '
    'style="width:100%;height:auto;display:block;'
    'background:rgba(0,0,0,.15);border-radius:8px"></svg>'
    '<div style="display:flex;justify-content:space-between;'
    'font-size:9px;color:var(--muted);margin-top:6px;padding:0 4px">'
    '<span>Maintenant</span><span>+6h</span><span>+24h</span>'
    '<span>+7j</span><span>+30j</span>'
    '</div>'
    '</div>'
    '</div>'
)

# Injecter la vue dans le panel avant </body>
NXC_PANEL_HTML = NXC_PANEL_HTML.replace(
    "\\n</script>\\n</body>\\n</html>\\n",
    "\\n</script>\\n" + _PREVISION_VIEW + "\\n</body>\\n</html>\\n",
    1
)
...
# ==============================================================================
# JS DE CALCUL PREVISION — fonction calcPrev()
# ==============================================================================

_PREVISION_JS = (
    "<script>"
    "function calcPrev(){"
    "var p0=parseFloat((mkt&&mkt.price)||5000);"
    "var drift=biasDrift||0;"
    "var sigma=0.015;"
    "var mrTarget=5000;"
    "var mpInp=$('mp-target');"
    "if(mpInp&&mpInp.value)mrTarget=parseFloat(mpInp.value)||5000;"
    "var elC=document.getElementById('pv-current');"
    "if(elC)elC.textContent=fmt(p0,0)+' R';"
    "var elT=document.getElementById('pv-target');"
    "if(elT)elT.textContent=fmt(mrTarget,0)+' R';"
    "var elTr=document.getElementById('pv-trend');"
    "if(elTr){"
    "if(drift>0.3){elTr.textContent='\U0001f680 Haussiere';elTr.style.color='var(--green)';}"
    "else if(drift<-0.3){elTr.textContent='\U0001f43b Baissiere';elTr.style.color='var(--red)';}"
    "else{elTr.textContent='⚖ Neutre';elTr.style.color='var(--muted)';}"
    "}"
    "var autoBias=0.0003;"
    "var biasPerTick=drift*0.05/2;"
    ...
    "];"
    "var rows=[];var svgPts=[{h:0,p:p0,lo:p0,hi:p0}];"
    "for(var i=0;i<horizons.length;i++){"
    "var hz=horizons[i];"
    "var nT=Math.round(hz.hours*240);"
    "var price=p0;var varAcc=0;"
    "for(var t=0;t<nT;t++){"
    "var mrP=mrActive?(mrTarget-price)/Math.max(price,1)*0.04:0;"
    "price*=(1+autoBias+biasPerTick+mrP);"
    "varAcc+=sigma*sigma;"
    "}"
    "price=Math.max(50,Math.min(999999,price));"
    "var std=Math.sqrt(varAcc)*price;"
    "var lo=Math.max(50,price-1.28*std);"
    "var hi=Math.min(999999,price+1.28*std);"
    "var pct=(price-p0)/p0*100;"
    "var sign=pct>=0?'+':'';"
    "var col=pct>5?'var(--green)':pct<-5?'var(--red)':'var(--muted)';"
    "rows.push("
    "'<tr style=\"border-bottom:1px solid rgba(255,255,255,.04)\">'"
    "+'<td style=\"padding:9px 6px;color:var(--cyan);font-weight:700\">'+hz.label+'</td>'"
    "+'<td style=\"padding:9px 6px;text-align:right;font-family:monospace;"
    "font-weight:700;color:'+col+'\">'+fmt(price,0)+' R</td>'"
    "+'<td style=\"padding:9px 6px;text-align:right;font-family:monospace;"
    "color:var(--red)\">'+fmt(lo,0)+'</td>'"
    "+'<td style=\"padding:9px 6px;text-align:right;font-family:monospace;"
    "color:var(--green)\">'+fmt(hi,0)+'</td>'"
    "+'<td style=\"padding:9px 6px;text-align:center;font-weight:700;color:"
    "'+col+'\">'+sign+pct.toFixed(1)+'%</td>'"
    "+'</tr>'"
    ");"
    "svgPts.push({h:hz.hours,p:price,lo:lo,hi:hi});"
    "}"
    ...
    "var pMin2=Math.min.apply(null,allP)*0.993;"
    "var pMax2=Math.max.apply(null,allP)*1.007;"
    "var W=320,H=110,P=14,maxH=720;"
    "function sx(h){return P+(h/maxH)*(W-2*P);}"
    "function sy(p){return P+(1-(p-pMin2)/(pMax2-pMin2+0.01))*(H-2*P);}"
    "var band='';"
    ...
    "var ty=sy(mrTarget).toFixed(1);"
    "var tL='M'+P+' '+ty+'L'+(W-P)+' '+ty;"
    ...
    "+'<path d=\"'+tL+'\" stroke=\"rgba(255,176,32,0.45)\" stroke-width=\"1\""
    "' stroke-dasharray=\"5 3\" fill=\"none\"/>'"
    "+'<path d=\"'+line+'\" stroke=\"'+lc+'\" stroke-width=\"2\" fill=\"none\""
    "' stroke-linejoin=\"round\" stroke-linecap=\"round\"/>'"
    "+'<circle cx=\"'+sx(0)+'\" cy=\"'+sy(p0)+'\" r=\"5\" fill=\"'+lc+'\"/>'"
    "+'<circle cx=\"'+sx(720)+'\" cy=\"'+sy(svgPts[svgPts.length-1].p)+'\"'"
    "' r=\"3\" fill=\"'+lc+'\" opacity=\"0.6\"/>';"
    "}"
    "</script>"
)

# Injecter le JS dans le panel avant </body>
NXC_PANEL_HTML = NXC_PANEL_HTML.replace(
    "\\n</script>\\n</body>\\n</html>\\n",
    "\\n</script>\\n" + _PREVISION_JS + "\\n</body>\\n</html>\\n",
    1
