...
@app.route("/nxc/bank/user", methods=["POST"])
def nxc_bank_user():
    """Opérations sur le compte R ou NXC d'un utilisateur spécifique."""
    body = request.get_json(force=True, silent=True) or {}
    ...
    op   = body.get("op", "")        # "add_r", "sub_r", "add_nxc", "sub_nxc", "set_r", "set_nxc"
    amount = float(body.get("amount", 0))
    if amount < 0:
        return jsonify({"ok": False, "error": "Montant negatif"}), 400
    with _lock:
        db = load_db()
        if username not in db.get("users", {}):
            return jsonify({"ok": False, "error": "Utilisateur introuvable"}), 404
        user = db["users"][username]
        bal_r   = float(user.get("balance", 0))
        bal_nxc = float(user.get("nxc_balance", 0))
        if op == "add_r":
            bal_r   += amount
        elif op == "sub_r":
            bal_r   = max(0.0, bal_r - amount)
        elif op == "set_r":
            bal_r   = max(0.0, amount)
        elif op == "add_nxc":
            bal_nxc += amount
        elif op == "sub_nxc":
            bal_nxc = max(0.0, bal_nxc - amount)
        elif op == "set_nxc":
            bal_nxc = max(0.0, amount)
        else:
            return jsonify({"ok": False, "error": "op invalide"}), 400
        user["balance"]     = round(bal_r, 4)
        user["nxc_balance"] = round(bal_nxc, 6)
        save_db(db)
    return jsonify({"ok": True, "username": username,
                    "balance": round(bal_r, 4),
                    "nxc_balance": round(bal_nxc, 6)})


@app.route("/nxc/interest", methods=["POST"])
def nxc_interest():
    """Applique des intérêts (%) sur les balances R ou NXC de tous les utilisateurs."""
    body = request.get_json(force=True, silent=True) or {}
    mk = body.get("master_key", "")
    if not mk or not secrets.compare_digest(mk, MASTER_KEY):
        return jsonify({"ok": False, "error": "Unauthorized"}), 403
    rate_r   = float(body.get("rate_r",   0))   # % appliqué aux R
    rate_nxc = float(body.get("rate_nxc", 0))   # % appliqué aux NXC
    min_bal  = float(body.get("min_bal",  0))    # ignorer balances < min_bal
    cap      = float(body.get("cap",      0))    # 0 = pas de plafond
    with _lock:
        db  = load_db()
        modified = 0
        total_r_delta   = 0.0
        ...
        for uname, u in db.get("users", {}).items():
            bal_r   = float(u.get("balance", 0))
            bal_nxc = float(u.get("nxc_balance", 0))
            changed = False
            if rate_r != 0 and bal_r >= min_bal:
                delta_r = bal_r * rate_r / 100.0
                if cap > 0: delta_r = min(delta_r, cap)
                bal_r  += delta_r
                total_r_delta += delta_r
                changed = True
            if rate_nxc != 0 and bal_nxc >= min_bal:
                delta_nxc = bal_nxc * rate_nxc / 100.0
                if cap > 0: delta_nxc = min(delta_nxc, cap)
                bal_nxc  += delta_nxc
                total_nxc_delta += delta_nxc
                changed = True
            if changed:
                u["balance"]     = round(max(0, bal_r),   4)
                u["nxc_balance"] = round(max(0, bal_nxc), 6)
                modified += 1
        if modified:
            save_db(db)
    return jsonify({"ok": True, "users_modified": modified,
                    "total_r_delta": round(total_r_delta, 4),
                    "total_nxc_delta": round(total_nxc_delta, 6)})


@app.route("/nxc/users/balances", methods=["POST"])
def nxc_users_balances():
    """Liste les balances de tous les utilisateurs (pour le panel admin NXC)."""
    body = request.get_json(force=True, silent=True) or {}
    mk = body.get("master_key", "")
    if not mk or not secrets.compare_digest(mk, MASTER_KEY):
        return jsonify({"ok": False, "error": "Unauthorized"}), 403
    with _lock:
        db = load_db()
        users = []
        p = float(NXC_MARKET.get("price", 1))
        for uname, u in db.get("users", {}).items():
            bal_r   = float(u.get("balance", 0))
            bal_nxc = float(u.get("nxc_balance", 0))
            users.append({
                ...
                "balance_r":   round(bal_r, 2),
                "balance_nxc": round(bal_nxc, 6),
                "value_r":     round(bal_nxc * p, 2),
                "total_wealth":round(bal_r + bal_nxc * p, 2)
            })
        users.sort(key=lambda x: -x["total_wealth"])
    return jsonify({"ok": True, "users": users, "count": len(users), "nxc_price": p})
