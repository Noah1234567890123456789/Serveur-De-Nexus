...
@app.route("/nxc/bank/print", methods=["GET", "POST"])
def nxc_bank_print():
    """Page HTML imprimable avec toutes les informations de la banque NXC."""
    body = request.get_json(force=True, silent=True) or {}
    ...
    try:
        with _lock:
            ...
            bank = noah.get("data", {}).get("nxcoin_bank",
                {"reserves": 0, "nxcEmis": 0, "totalIn": 0, "totalOut": 0, "flux": []})
        reserves   = float(bank.get("reserves", 0))
        nxc_emis   = float(bank.get("nxcEmis", 0))
        total_in   = float(bank.get("totalIn", 0))
        total_out  = float(bank.get("totalOut", 0))
        flux_list  = bank.get("flux", []) or []
        nb_flux    = len(flux_list)
        prix_nxc   = float(NXC_MARKET.get("price", 0))
        val_stock  = round(nxc_emis * prix_nxc, 2)
        benefice   = round(total_in - total_out, 2)
        ratio_pct  = round((total_out / total_in * 100) if total_in > 0 else 0.0, 1)
        now_str    = datetime.datetime.now().strftime("%d/%m/%Y à %H:%M:%S")
        flux_rows  = ""
        for i, f in enumerate(reversed(flux_list[-200:]), 1):
            ftype  = f.get("type", "?")
            famt   = f.get("amount", 0)
            fts    = f.get("ts", 0)
            fnote  = f.get("note", "")
            fdate  = datetime.datetime.fromtimestamp(fts / 1000).strftime("%d/%m/%Y %H:%M") if fts else "—"
            is_in  = ftype in ("in", "entrée", "injection", "inject")
            color  = "#1a8a4a" if is_in else "#c0392b"
            signe  = "+" if is_in else "−"
            flux_rows += (
                "<tr>"
                "<td style='color:#8a96ad'>" + str(i) + "</td>"
                "<td>" + fdate + "</td>"
                "<td style='color:" + color + ";font-weight:700'>" + signe + " " + str(famt) + " R</td>"
                "<td style='color:#6b2fd0'>" + ftype + "</td>"
                "<td style='color:#555'>" + fnote + "</td>"
                "</tr>"
            )
        if not flux_rows:
            flux_rows = "<tr><td colspan='5' style='color:#aaa;text-align:center'>Aucun flux enregistré</td></tr>"
        signe_b = "+" if benefice >= 0 else ""
        class_b = "green" if benefice >= 0 else "red"
        html = (
            "<!DOCTYPE html>"
            "<html lang='fr'><head><meta charset='utf-8'>"
            "<title>Rapport Banque NXC — " + now_str + "</title>"
            "<style>"
            "* { box-sizing:border-box; font-family:'Segoe UI',Arial,sans-serif; }"
            "body { margin:0; background:#fff; color:#111; padding:32px; }"
            "h1 { font-size:26px; font-weight:800; letter-spacing:1px; margin:0 0 4px; color:#3a0ab5; }"
            ...
            ".card { border:1.5px solid #e0e0f0; border-radius:12px; padding:14px 18px; background:#f7f7ff; }"
            ".card .lbl { font-size:11px; text-transform:uppercase; letter-spacing:.8px; color:#888; margin-bottom:4px; }"
            ".card .val { font-size:22px; font-weight:700; color:#1a1a3a; }"
            ".green .val { color:#1a8a4a; } .red .val { color:#c0392b; } .purple .val { color:#6b2fd0; }"
            ".stitle { font-size:13px; text-transform:uppercase; letter-spacing:1px; color:#888;"
            "  margin:24px 0 8px; border-bottom:1px solid #e0e0f0; padding-bottom:6px; }"
            "table { width:100%; border-collapse:collapse; font-size:13px; }"
            "th { background:#f0eeff; color:#555; font-weight:600; padding:8px 10px; text-align:left;"
            "  border-bottom:2px solid #d0c8f0; }"
            ...
            "</style></head><body>"
            "<h1>◈ BANQUE NXC — RAPPORT COMPLET</h1>"
            "<div class='sub'>Généré le " + now_str + " &nbsp;|&nbsp; Prix NXC : " + "{:,.2f}".format(prix_nxc) + " R</div>"
            "<div class='grid'>"
            "<div class='card'><div class='lbl'>Réserves</div><div class='val'>" + "{:,.2f}".format(reserves) + " R</div></div>"
            "<div class='card green'><div class='lbl'>Total entré</div><div class='val'>+" + "{:,.2f}".format(total_in) + " R</div></div>"
            "<div class='card red'><div class='lbl'>Total sorti</div><div class='val'>−" + "{:,.2f}".format(total_out) + " R</div></div>"
            "<div class='card'><div class='lbl'>Ratio (sorti / entré)</div><div class='val'>" + "{:.1f}".format(ratio_pct) + " %</div></div>"
            "<div class='card purple'><div class='lbl'>NXC émis</div><div class='val'>" + "{:,.4f}".format(nxc_emis) + " NXC</div></div>"
            "<div class='card'><div class='lbl'>Val. stock (NXC × prix)</div><div class='val'>" + "{:,.2f}".format(val_stock) + " R</div></div>"
            "<div class='card " + class_b + "'><div class='lbl'>Bénéfice (entré − sorti)</div><div class='val'>" + signe_b + "{:,.2f}".format(benefice) + " R</div></div>"
            "<div class='card'><div class='lbl'>Nb flux</div><div class='val'>" + str(nb_flux) + "</div></div>"
            "<div class='card'><div class='lbl'>Prix NXC actuel</div><div class='val'>" + "{:,.2f}".format(prix_nxc) + " R</div></div>"
            "</div>"
            "<div class='stitle'>◈ FLUX (" + str(nb_flux) + " entrée(s)/sortie(s))</div>"
            "<table><thead><tr><th>#</th><th>Date</th><th>Montant</th><th>Type</th><th>Note</th></tr></thead>"
            "<tbody>" + flux_rows + "</tbody></table>"
            "<div class='footer'>Nexus Bank Report &nbsp;·&nbsp; " + now_str + "</div>"
            "<scri" + "pt>window.print();</" + "script>"
            "</body></html>"
        )
...
@app.route("/nxc/prevision", methods=["GET", "POST"])
def nxc_prevision():
    """Page de prevision NXC — simulation Monte Carlo + graphiques interactifs."""
    body = request.get_json(force=True, silent=True) or {}
    mk = (body.get("master_key") or request.args.get("master_key") or "")
    if not mk or not secrets.compare_digest(mk, MASTER_KEY):
        return ("Acces refuse", 403)
    import json as _json
    prix_actuel = round(NXC_MARKET["price"], 2)
    data_js = _json.dumps({
        "prix": prix_actuel,
        "floor": 50.0,
        "ceiling": 100000.0,
        ...
        "soft_ceil": 80000.0,
        "tick_s": 15,
        "history": NXC_MARKET.get("history", [])[-100:]
    })

    H = (
        "<!DOCTYPE html><html lang='fr'><head><meta charset='utf-8'>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'>"
        "<title>📈 NXC Prévision</title>"
        "<script src='https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js'></script>"
        "<script src='https://cdnjs.cloudflare.com/ajax/libs/hammer.js/2.0.8/hammer.min.js'></script>"
        "<script src='https://cdnjs.cloudflare.com/ajax/libs/chartjs-plugin-zoom/1.2.1/chartjs-plugin-zoom.min.js'></script>"
        "<style>"
        ":root{--bg:#05080f;--bg2:#0c1120;--bg3:#131a2e;--brd:rgba(100,160,255,.14);--acc:#3d7bff;"
        "--cy:#00e5ff;--gr:#00ff9d;--rd:#ff3d5e;--pu:#a06bff;--go:#ffb020;--tx:#e8edfb;--mt:#5c6b8c}"
        "*{box-sizing:border-box;margin:0;padding:0}"
        "body{background:var(--bg);color:var(--tx);font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh;"
        "background-image:radial-gradient(ellipse 80% 40% at 50% 0%,rgba(61,123,255,.07),transparent)}"
        ".hdr{background:rgba(12,17,32,.97);border-bottom:1px solid var(--brd);padding:14px 22px;"
        "display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:99;backdrop-filter:blur(20px)}"
        ".hdr-title{font-size:18px;font-weight:800;background:linear-gradient(90deg,var(--cy),var(--acc),var(--pu));"
        "-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:1px}"
        ".hdr-price{background:rgba(0,229,255,.1);border:1px solid rgba(0,229,255,.25);border-radius:9px;"
        "padding:5px 13px;font-size:13px;font-weight:700;color:var(--cy)}"
        ".back-btn{margin-left:auto;padding:6px 14px;border-radius:8px;background:rgba(255,255,255,.04);"
        "border:1px solid var(--brd);color:var(--mt);font-size:12px;cursor:pointer;text-decoration:none}"
        ".back-btn:hover{color:var(--tx)}"
        ".ctrl{background:var(--bg2);border-bottom:1px solid var(--brd);padding:18px 22px;"
        "display:flex;flex-wrap:wrap;gap:14px;align-items:flex-end}"
        ".cg{display:flex;flex-direction:column;gap:6px}"
        ".cl{font-size:11px;color:var(--mt);text-transform:uppercase;letter-spacing:.7px;"
        "display:flex;align-items:center;gap:5px}"
        ".ci{background:rgba(0,0,0,.4);border:1px solid var(--brd);border-radius:9px;color:var(--tx);"
        "padding:9px 13px;font-size:14px;outline:none;font-family:inherit;transition:border-color .2s}"
        ".ci:focus{border-color:var(--acc)}"
        ".ps{display:flex;gap:5px;flex-wrap:wrap}"
        ".pb{padding:6px 11px;border-radius:7px;background:rgba(61,123,255,.07);border:1px solid var(--brd);"
        "color:var(--tx);font-size:12px;cursor:pointer;transition:all .15s}"
        ".pb:hover,.pb.on{background:rgba(61,123,255,.22);border-color:var(--acc);color:var(--cy)}"
        ".run{padding:11px 26px;border-radius:11px;background:linear-gradient(135deg,var(--acc),var(--pu));"
        "border:none;color:#fff;font-size:14px;font-weight:700;cursor:pointer;"
        "box-shadow:0 6px 20px rgba(61,123,255,.28);transition:all .2s}"
        ".run:hover{transform:translateY(-2px);box-shadow:0 10px 28px rgba(61,123,255,.38)}"
        ".run:disabled{opacity:.45;transform:none;cursor:not-allowed}"
        ".pbar{padding:14px 22px;display:flex;flex-wrap:wrap;gap:10px;"
        "background:rgba(0,0,0,.18);border-bottom:1px solid var(--brd)}"
        ".pc{flex:1;min-width:130px;background:var(--bg2);border:1px solid var(--brd);"
        "border-radius:11px;padding:11px 15px}"
        ".pl{font-size:10px;text-transform:uppercase;letter-spacing:.7px;color:var(--mt);margin-bottom:3px}"
        ".pv{font-size:19px;font-weight:700}"
        ".up{color:var(--gr)}.dn{color:var(--rd)}.ne{color:var(--cy)}"
        ".params{margin:0 22px 18px;background:rgba(61,123,255,.04);border:1px solid rgba(61,123,255,.13);"
        "border-radius:11px;padding:12px 17px;font-size:12px;color:var(--mt);"
        "display:flex;flex-wrap:wrap;gap:12px}"
        ".pi{display:flex;flex-direction:column;gap:2px}"
        ".pk{font-size:10px;text-transform:uppercase;letter-spacing:.6px;color:rgba(100,160,255,.55)}"
        ".pv2{font-size:13px;font-weight:700;color:var(--tx)}"
        ".grid{padding:18px 22px;display:grid;grid-template-columns:1fr 1fr;gap:18px}"
        "@media(max-width:860px){.grid{grid-template-columns:1fr}}"
        ".cc{background:var(--bg2);border:1px solid var(--brd);border-radius:15px;padding:15px}"
        ".cc.wide{grid-column:1/-1}"
        ".ch{display:flex;align-items:center;gap:7px;margin-bottom:11px}"
        ".ct{font-size:12px;font-weight:700;flex:1}"
        ".ib{width:20px;height:20px;border-radius:50%;background:rgba(61,123,255,.13);"
        "border:1px solid var(--brd);color:var(--acc);font-size:10px;font-weight:700;"
        "cursor:pointer;display:flex;align-items:center;justify-content:center;position:relative;flex-shrink:0}"
        ".ib:hover .tt{display:block}"
        ".tt{display:none;position:absolute;bottom:26px;right:0;background:var(--bg3);"
        "border:1px solid var(--brd);border-radius:9px;padding:9px 11px;font-size:11px;"
        "color:var(--mt);width:210px;line-height:1.5;z-index:10;box-shadow:0 8px 20px rgba(0,0,0,.5);"
        "white-space:normal;text-align:left}"
        ".zr{padding:3px 9px;border-radius:6px;background:rgba(255,255,255,.04);"
        "border:1px solid var(--brd);color:var(--mt);font-size:11px;cursor:pointer}"
        ".zr:hover{color:var(--tx)}"
        ".cw{position:relative;height:250px}"
        ".cw canvas{max-height:250px}"
        ".ae{display:flex;gap:7px;margin-top:9px;flex-wrap:wrap;align-items:center}"
        ".ae span{font-size:11px;color:var(--mt)}"
        ".ae input{width:75px;background:rgba(0,0,0,.3);border:1px solid var(--brd);"
        "border-radius:5px;color:var(--tx);padding:3px 6px;font-size:11px;outline:none}"
        ".ab{padding:3px 8px;border-radius:5px;background:rgba(61,123,255,.13);"
        "border:1px solid var(--brd);color:var(--acc);font-size:11px;cursor:pointer}"
        ".lo{display:none;position:fixed;inset:0;background:rgba(5,8,15,.88);z-index:200;"
        "align-items:center;justify-content:center;flex-direction:column;gap:14px;backdrop-filter:blur(5px)}"
        ".lo.on{display:flex}"
        ".sp{width:44px;height:44px;border:3px solid rgba(61,123,255,.2);border-top-color:var(--acc);"
        "border-radius:50%;animation:sp 1s linear infinite}"
        "@keyframes sp{to{transform:rotate(360deg)}}"
        ".lt{color:var(--mt);font-size:13px}"
        ".sep{margin:4px 22px 0;border-top:1px solid var(--brd)}"
        "</style></head><body>"
        "<div class='lo' id='lo'><div class='sp'></div><div class='lt' id='lt'>Simulation…</div></div>"
        "<div class='hdr'>"
        "<div style='font-size:22px'>📈</div>"
        "<div class='hdr-title'>NXC PRÉVISION</div>"
        "<div class='hdr-price' id='hp'>—</div>"
        "<a href='javascript:history.back()' class='back-btn'>← Retour</a>"
        "</div>"
        "<div class='ctrl'>"
        "<div class='cg'>"
        "<div class='cl'>Durée de prévision"
        "<span class='ib' style='position:relative'>i<span class='tt'>Horizon temporel de la simulation. Plus la durée est longue, plus l'incertitude augmente exponentiellement.</span></span></div>"
        "<div class='ps'>"
        "<button class='pb' onclick='setDur(30,this)'>30s</button>"
        "<button class='pb' onclick='setDur(300,this)'>5 min</button>"
        "<button class='pb' onclick='setDur(3600,this)'>1 h</button>"
        "<button class='pb on' onclick='setDur(86400,this)'>1 jour</button>"
        "<button class='pb' onclick='setDur(604800,this)'>1 sem.</button>"
        "<button class='pb' onclick='setDur(2592000,this)'>1 mois</button>"
        "<button class='pb' onclick='setDur(31536000,this)'>1 an</button>"
        "</div></div>"
        "<div class='cg'><div class='cl'>Durée personnalisée</div>"
        "<div style='display:flex;gap:6px'>"
        "<input class='ci' type='number' id='cd' placeholder='ex: 48' min='1' style='width:100px'>"
        "<select class='ci' id='cu' style='padding:9px 10px'>"
        "<option value='1'>Secondes</option><option value='60'>Minutes</option>"
        "<option value='3600' selected>Heures</option><option value='86400'>Jours</option>"
        "</select>"
        "<button class='pb' onclick='applyCustom()'>OK</button>"
        "</div></div>"
        "<div class='cg'><div class='cl'>Simulations"
        "<span class='ib' style='position:relative'>i<span class='tt'>Nombre de chemins Monte Carlo. Plus = précision accrue. 500 = bon équilibre vitesse/précision.</span></span></div>"
        "<select class='ci' id='ns'>"
        "<option value='200'>200 (rapide)</option>"
        "<option value='500' selected>500 (équilibré)</option>"
        "<option value='1000'>1 000 (précis)</option>"
        "<option value='2000'>2 000 (très précis)</option>"
        "</select></div>"
        "<button class='run' onclick='runAll()' id='rb'>▶ Simuler</button>"
        "</div>"
        "<div class='params' id='pm'>"
        "<div class='pi'><div class='pk'>Prix actuel</div><div class='pv2' id='pp'>—</div></div>"
        "<div class='pi'><div class='pk'>Plancher dur</div><div class='pv2' style='color:var(--rd)'>50 R</div></div>"
        "<div class='pi'><div class='pk'>Plancher doux</div><div class='pv2' style='color:var(--go)'>200 R</div></div>"
        "<div class='pi'><div class='pk'>Plafond doux</div><div class='pv2' style='color:var(--go)'>80 000 R</div></div>"
        "<div class='pi'><div class='pk'>Plafond dur</div><div class='pv2' style='color:var(--rd)'>100 000 R</div></div>"
        "<div class='pi'><div class='pk'>Tick</div><div class='pv2'>15 s</div></div>"
        "<div class='pi'><div class='pk'>Durée</div><div class='pv2' id='pd'>1 jour</div></div>"
        "<div class='pi'><div class='pk'>Ticks total</div><div class='pv2' id='pt'>5 760</div></div>"
        "</div>"
        "<div class='pbar' id='sb' style='display:none'>"
        "<div class='pc'><div class='pl'>Prix médian</div><div class='pv ne' id='s1'>—</div></div>"
        "<div class='pc'><div class='pl'>P5 pessimiste</div><div class='pv dn' id='s2'>—</div></div>"
        "<div class='pc'><div class='pl'>P95 optimiste</div><div class='pv up' id='s3'>—</div></div>"
        "<div class='pc'><div class='pl'>Prob. hausse</div><div class='pv up' id='s4'>—</div></div>"
        "<div class='pc'><div class='pl'>Prob. < plancher doux</div><div class='pv dn' id='s5'>—</div></div>"
        "<div class='pc'><div class='pl'>Prob. > plafond doux</div><div class='pv up' id='s6'>—</div></div>"
        "<div class='pc'><div class='pl'>Volatilité moy.</div><div class='pv ne' id='s7'>—</div></div>"
        "<div class='pc'><div class='pl'>Espérance</div><div class='pv ne' id='s8'>—</div></div>"
        "</div>"
        "<div class='sep'></div>"
        "<div class='grid'>"

        # Chart 1 - Monte Carlo wide
        "<div class='cc wide'>"
        "<div class='ch'>"
        "<div class='ct'>📊 Monte Carlo — Chemins de prix + percentiles"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>10 chemins aléatoires (translucides) + bandes P5/P25/médiane/P75/P95. Molette = zoom vertical, cliquer-glisser = pan. La ligne orange = médiane de tous les chemins.</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c1\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c1'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c1ymi' placeholder='auto'>"
        "<span>Y max</span><input type='number' id='c1yma' placeholder='auto'>"
        "<span>X labels</span><input type='number' id='c1xt' placeholder='8' min='2' max='50'>"
        "<button class='ab' onclick='aa(\"c1\",\"c1ymi\",\"c1yma\",\"c1xt\")'>Appliquer</button>"
        "</div>"
        "</div>"
        ...
        "<div class='cc'>"
        "<div class='ch'>"
        "<div class='ct'>📉 Distribution des prix finaux"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>Histogramme des prix finaux à l'échéance. Bleu = zone normale, rouge = sous plancher doux, or = sur plafond doux. Plus une barre est haute, plus ce scénario est probable.</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c2\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c2'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c2ymi' placeholder='0'>"
        "<span>Y max</span><input type='number' id='c2yma' placeholder='auto'>"
        "<button class='ab' onclick='aa(\"c2\",\"c2ymi\",\"c2yma\",null)'>Appliquer</button>"
        "</div>"
        "</div>"

        # Chart 3 - Zone probabilities
        "<div class='cc'>"
        "<div class='ch'>"
        "<div class='ct'>🗺️ Probabilité par zone au fil du temps"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>À chaque instant : % de chemins en zone normale (vert), sous le plancher doux (rouge) ou au-dessus du plafond doux (or). La somme = 100%.</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c3\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c3'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c3ymi' placeholder='0'>"
        "<span>Y max</span><input type='number' id='c3yma' placeholder='100'>"
        "<button class='ab' onclick='aa(\"c3\",\"c3ymi\",\"c3yma\",null)'>Appliquer</button>"
        "</div>"
        "</div>"

        # Chart 4 - Confidence envelope wide
        "<div class='cc wide'>"
        "<div class='ch'>"
        "<div class='ct'>📐 Enveloppe de confiance — Min · P25 · Médiane · P75 · Max"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>Zone bleue = intervalle entre P25 et P75 (50% des scénarios). Orange = médiane. Violet = moyenne. Les tirets verts/rouges = extrêmes absolus observés sur toutes les simulations.</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c4\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c4'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c4ymi' placeholder='auto'>"
        "<span>Y max</span><input type='number' id='c4yma' placeholder='auto'>"
        "<button class='ab' onclick='aa(\"c4\",\"c4ymi\",\"c4yma\",null)'>Appliquer</button>"
        "</div>"
        "</div>"
        ...
        "<div class='cc'>"
        "<div class='ch'>"
        "<div class='ct'>⚡ Volatilité glissante (% / tick)"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>Écart-type des log-retours par fenêtre temporelle. Mesure l'instabilité du marché à chaque instant. La ligne pointillée rouge = volatilité théorique de base (~1.15% par tick).</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c5\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c5'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c5ymi' placeholder='0'>"
        "<span>Y max</span><input type='number' id='c5yma' placeholder='auto'>"
        "<button class='ab' onclick='aa(\"c5\",\"c5ymi\",\"c5yma\",null)'>Appliquer</button>"
        "</div>"
        "</div>"
        ...
        "<div class='cc'>"
        "<div class='ch'>"
        "<div class='ct'>🎯 Momentum — Ratio haussier par fenêtre"
        "<span class='ib' style='position:relative;margin-left:5px'>i<span class='tt'>% de ticks positifs par fenêtre temporelle. L'algorithme a un biais de -0.02 (48% de hausse théorique). Vert = plus de hausses que prévu, rouge = tendance baissière.</span></span>"
        "</div>"
        "<button class='zr' onclick='rz(\"c6\")'>↺ zoom</button>"
        "</div>"
        "<div class='cw'><canvas id='c6'></canvas></div>"
        "<div class='ae'>"
        "<span>Y min</span><input type='number' id='c6ymi' placeholder='0'>"
        "<span>Y max</span><input type='number' id='c6yma' placeholder='100'>"
        "<button class='ab' onclick='aa(\"c6\",\"c6ymi\",\"c6yma\",null)'>Appliquer</button>"
        "</div>"
        "</div>"
    ...
    JS_TAIL = (
        ";"
        "const FL=D.floor,CE=D.ceiling,SF=D.soft_floor,SC=D.soft_ceil,TS=D.tick_s;"
        "let price=D.prix,durS=86400,charts={};"
        "document.getElementById('hp').textContent=fmt(price)+' R';"
        "document.getElementById('pp').textContent=fmt(price)+' R';"
        "updParams();"

        "function fmt(v,d=2){return parseFloat(v).toLocaleString('fr-FR',{minimumFractionDigits:d,maximumFractionDigits:d});}"
        "function fmtS(v){if(v>=1e6)return (v/1e6).toFixed(2)+' M';if(v>=1000)return (v/1000).toFixed(1)+' k';return fmt(v);}"
        "function fmtD(s){"
        "if(s<60)return s+'s';"
        "if(s<3600)return (s/60).toFixed(1)+' min';"
        "if(s<86400)return (s/3600).toFixed(1)+'h';"
        "if(s<604800)return (s/86400).toFixed(1)+' j';"
        "if(s<2592000)return (s/604800).toFixed(1)+' sem.';"
        "if(s<31536000)return (s/2592000).toFixed(1)+' mois';"
        "return (s/31536000).toFixed(2)+' an(s)';}"
        ...
        "const v=parseFloat(document.getElementById('cd').value);"
        "const u=parseFloat(document.getElementById('cu').value);"
        "if(v>0&&!isNaN(v))setDur(v*u,null);}"

        "function updParams(){"
        "const t=Math.max(1,Math.ceil(durS/TS));"
        "document.getElementById('pd').textContent=fmtD(durS);"
        "document.getElementById('pt').textContent=t.toLocaleString('fr-FR')+' ticks';}"

        # Simulation
        "function simPath(p0,n){"
        "const a=new Float64Array(n+1);a[0]=p0;let p=p0;"
        "for(let i=1;i<=n;i++){"
        "const sg=0.008+Math.random()*0.015;"
        "let dj=(Math.random()-0.48)*sg;"
        "if(p>SC)dj-=0.012;"
        "if(p<SF)dj+=0.018;"
        "p=Math.max(FL,Math.min(CE,p*(1+dj)));"
        "a[i]=p;}return a;}"
        ...
        "function computeStats(paths,nSteps){"
        "const maxPts=Math.min(nSteps+1,180);"
        "const step=Math.max(1,Math.floor((nSteps+1)/maxPts));"
        "const idx=[];"
        "for(let i=0;i<=nSteps;i+=step)idx.push(i);"
        "if(idx[idx.length-1]!==nSteps)idx.push(nSteps);"
        "const nP=paths.length;"
        "const p5=[],p25=[],p50=[],p75=[],p95=[],pm=[],pn=[],px=[];"
        "const zf=[],zn=[],zc=[],vol=[];"
        "for(const ti of idx){"
        "const v=paths.map(p=>p[ti]);"
        "p5.push(pct(v,5));p25.push(pct(v,25));p50.push(pct(v,50));"
        "p75.push(pct(v,75));p95.push(pct(v,95));"
        "pm.push(v.reduce((a,b)=>a+b,0)/nP);"
        "pn.push(Math.min(...v));px.push(Math.max(...v));"
        "const nf=v.filter(x=>x<SF).length,nc=v.filter(x=>x>SC).length;"
        "zf.push(nf/nP*100);zc.push(nc/nP*100);zn.push((nP-nf-nc)/nP*100);"
        "if(ti>0){"
        "const lr=paths.map(p=>{const pv=p[Math.max(0,ti-step)];return pv>0?Math.log(p[ti]/pv):0;});"
        "const mu=lr.reduce((a,b)=>a+b,0)/nP;"
        "vol.push(Math.sqrt(lr.reduce((a,b)=>a+(b-mu)**2,0)/nP)*100);"
        "}else vol.push(0);}"
        # Momentum
        "const ws=Math.max(1,Math.floor(nSteps/20));"
        "const ml=[],mv=[];"
        "for(let w=ws;w<=nSteps;w+=ws){"
        "let up=0,tot=0;"
        "for(const p of paths)for(let i=Math.max(1,w-ws+1);i<=w&&i<=nSteps;i++){tot++;if(p[i]>p[i-1])up++;}"
        "ml.push(fmtD((w/nSteps)*durS));mv.push(tot>0?up/tot*100:50);}"
        # Histogram
        "const fp=paths.map(p=>p[nSteps]);"
        "const bins=30,hmi=Math.min(...fp),hma=Math.max(...fp);"
        "const bw=(hma-hmi)/bins||1,hc=new Array(bins).fill(0);"
        "fp.forEach(v=>{const b=Math.min(bins-1,Math.floor((v-hmi)/bw));hc[b]++;});"
        "const hl=Array.from({length:bins},(_,i)=>fmtS(hmi+i*bw));"
        "const hcol=hc.map((_,i)=>{const m=hmi+(i+.5)*bw;"
        "return m<SF?'rgba(255,61,94,.7)':m>SC?'rgba(255,176,32,.7)':'rgba(61,123,255,.7)';});"
        "const tl=idx.map(ti=>fmtD(ti*TS));"
        "const med=pct(fp,50),p5f=pct(fp,5),p95f=pct(fp,95);"
        "const pUp=fp.filter(v=>v>price).length/nP*100;"
        "const pFl=fp.filter(v=>v<SF).length/nP*100;"
        "const pCe=fp.filter(v=>v>SC).length/nP*100;"
        "const avgV=vol.filter(v=>v>0).reduce((a,b)=>a+b,0)/Math.max(1,vol.filter(v=>v>0).length);"
        "const esp=fp.reduce((a,b)=>a+b,0)/nP;"
        "return {tl,p5,p25,p50,p75,p95,pm,pn,px,zf,zn,zc,vol,ml,mv,hl,hc,hcol,fp,"
        "med,p5f,p95f,pUp,pFl,pCe,avgV,esp};}"

        # Chart helpers
        "const CD={responsive:true,maintainAspectRatio:false,animation:{duration:300},"
        "plugins:{legend:{labels:{color:'#8a96b8',font:{size:10},boxWidth:12,padding:10}},"
        "zoom:{zoom:{wheel:{enabled:true},pinch:{enabled:true},mode:'xy'},pan:{enabled:true,mode:'xy'}}},"
        "scales:{"
        "x:{grid:{color:'rgba(100,140,255,.05)'},ticks:{color:'#5c6b8c',font:{size:9},maxTicksLimit:8}},"
        "y:{grid:{color:'rgba(100,140,255,.05)'},ticks:{color:'#5c6b8c',font:{size:9},maxTicksLimit:8}}}};"

        "function mk(id,cfg){"
        "if(charts[id])charts[id].destroy();"
        "const c=document.getElementById(id).getContext('2d');"
        "charts[id]=new Chart(c,cfg);return charts[id];}"

        "function rz(id){if(charts[id])charts[id].resetZoom();}"

        "function aa(cid,ymi,yma,xt){"
        "const c=charts[cid];if(!c)return;"
        "const yn=ymi?parseFloat(document.getElementById(ymi).value):NaN;"
        "const yx=yma?parseFloat(document.getElementById(yma).value):NaN;"
        "const xt2=xt?parseInt(document.getElementById(xt).value):NaN;"
        "if(!isNaN(yn))c.options.scales.y.min=yn;else delete c.options.scales.y.min;"
        "if(!isNaN(yx))c.options.scales.y.max=yx;else delete c.options.scales.y.max;"
        "if(!isNaN(xt2)&&xt2>0)c.options.scales.x.ticks.maxTicksLimit=xt2;"
        "c.update();}"

        "const PAL=['rgba(61,123,255,.35)','rgba(0,229,255,.35)','rgba(160,107,255,.35)',"
        "'rgba(0,255,157,.35)','rgba(255,176,32,.35)','rgba(255,61,94,.35)',"
        "'rgba(100,200,255,.35)','rgba(200,100,255,.35)','rgba(255,140,60,.35)','rgba(60,255,140,.35)'];"

        "function getSamples(paths,n=10){"
        "const st=Math.max(1,Math.floor(paths.length/n));"
        "return Array.from({length:n},(_,i)=>paths[i*st]);}"

        # Main run
        "async function runAll(){"
        "const nP=parseInt(document.getElementById('ns').value);"
        "const nS=Math.max(1,Math.ceil(durS/TS));"
        "document.getElementById('lo').classList.add('on');"
        "document.getElementById('lt').textContent='Simulation de '+nP+' chemins ('+nS.toLocaleString('fr-FR')+' ticks)…';"
        "document.getElementById('rb').disabled=true;"
        "await new Promise(r=>setTimeout(r,40));"
        "try{"
        "const paths=[];"
        "for(let i=0;i<nP;i++)paths.push(simPath(price,nS));"
        "const st=computeStats(paths,nS);"
        # Update summary
        "document.getElementById('sb').style.display='flex';"
        "document.getElementById('s1').textContent=fmt(st.med)+' R';"
        "document.getElementById('s2').textContent=fmt(st.p5f)+' R';"
        "document.getElementById('s3').textContent=fmt(st.p95f)+' R';"
        "document.getElementById('s4').textContent=st.pUp.toFixed(1)+'%';"
        "document.getElementById('s5').textContent=st.pFl.toFixed(1)+'%';"
        "document.getElementById('s6').textContent=st.pCe.toFixed(1)+'%';"
        "document.getElementById('s7').textContent=st.avgV.toFixed(3)+'% / tick';"
        "document.getElementById('s8').textContent=fmt(st.esp)+' R';"
        # Chart 1
        "const smp=getSamples(paths);"
        "const sIdx=[];"
        "const ss=Math.max(1,Math.floor((nS+1)/180));"
        "for(let i=0;i<=nS;i+=ss)sIdx.push(i);"
        "if(sIdx[sIdx.length-1]!==nS)sIdx.push(nS);"
        "const sLbl=sIdx.map(ti=>fmtD(ti*TS));"
        "const ds1=smp.map((p,i)=>({label:i===0?'Chemins simulés (10)':undefined,"
        "data:sIdx.map(ti=>p[ti]),borderColor:PAL[i%10],borderWidth:1,pointRadius:0,fill:false,tension:.1,showLine:true}));"
        "ds1.push({label:'P95',data:st.p95,borderColor:'rgba(0,255,157,.9)',borderWidth:2,pointRadius:0,fill:false,tension:.2});"
        "ds1.push({label:'P75',data:st.p75,borderColor:'rgba(0,229,255,.65)',borderWidth:1.5,pointRadius:0,fill:false,tension:.2,borderDash:[4,2]});"
        "ds1.push({label:'Médiane',data:st.p50,borderColor:'#ffb020',borderWidth:2.5,pointRadius:0,fill:false,tension:.2});"
        "ds1.push({label:'P25',data:st.p25,borderColor:'rgba(0,229,255,.65)',borderWidth:1.5,pointRadius:0,fill:false,tension:.2,borderDash:[4,2]});"
        "ds1.push({label:'P5',data:st.p5,borderColor:'rgba(255,61,94,.9)',borderWidth:2,pointRadius:0,fill:false,tension:.2});"
        "mk('c1',{type:'line',data:{labels:st.tl,datasets:ds1},options:JSON.parse(JSON.stringify(CD))});"
        # Chart 2
        "mk('c2',{type:'bar',data:{labels:st.hl,datasets:[{label:'# scénarios',"
        "data:st.hc,backgroundColor:st.hcol,borderRadius:3}]},"
        "options:{...JSON.parse(JSON.stringify(CD)),plugins:{...JSON.parse(JSON.stringify(CD)).plugins,legend:{display:false}}}});"
        # Chart 3
        "const cd3=JSON.parse(JSON.stringify(CD));"
        "cd3.scales.y.min=0;cd3.scales.y.max=100;"
        "cd3.scales.y.ticks={color:'#5c6b8c',font:{size:9},callback:v=>v+'%'};"
        "mk('c3',{type:'line',data:{labels:st.tl,datasets:["
        "{label:'% zone normale',data:st.zn,borderColor:'#00ff9d',backgroundColor:'rgba(0,255,157,.1)',fill:true,borderWidth:2,pointRadius:0,tension:.3},"
        "{label:'% > plafond doux',data:st.zc,borderColor:'#ffb020',backgroundColor:'rgba(255,176,32,.1)',fill:true,borderWidth:2,pointRadius:0,tension:.3},"
        "{label:'% < plancher doux',data:st.zf,borderColor:'#ff3d5e',backgroundColor:'rgba(255,61,94,.1)',fill:true,borderWidth:2,pointRadius:0,tension:.3}"
        "]},options:cd3});"
        # Chart 4
        "mk('c4',{type:'line',data:{labels:st.tl,datasets:["
        "{label:'Maximum',data:st.px,borderColor:'rgba(255,61,94,.4)',borderWidth:1,pointRadius:0,fill:false,tension:.2,borderDash:[2,4]},"
        "{label:'P75',data:st.p75,borderColor:'rgba(61,123,255,.5)',backgroundColor:'rgba(61,123,255,.07)',borderWidth:1.5,pointRadius:0,fill:'+1',tension:.2},"
        "{label:'P25',data:st.p25,borderColor:'rgba(61,123,255,.5)',borderWidth:1.5,pointRadius:0,fill:false,tension:.2},"
        "{label:'Médiane',data:st.p50,borderColor:'#ffb020',borderWidth:2.5,pointRadius:0,fill:false,tension:.2},"
        "{label:'Moyenne',data:st.pm,borderColor:'#a06bff',borderWidth:1.5,pointRadius:0,fill:false,tension:.2,borderDash:[4,2]},"
        "{label:'Minimum',data:st.pn,borderColor:'rgba(0,255,157,.4)',borderWidth:1,pointRadius:0,fill:false,tension:.2,borderDash:[2,4]}"
        "]},options:JSON.parse(JSON.stringify(CD))});"
        # Chart 5
        "const bv=(0.008+0.0075)*0.52*100;"
        "const cd5=JSON.parse(JSON.stringify(CD));"
        "cd5.scales.y.min=0;"
        "cd5.scales.y.ticks={color:'#5c6b8c',font:{size:9},callback:v=>v.toFixed(3)+'%'};"
        "mk('c5',{type:'line',data:{labels:st.tl,datasets:["
        "{label:'Volatilité',data:st.vol,borderColor:'#00e5ff',backgroundColor:'rgba(0,229,255,.07)',fill:true,borderWidth:2,pointRadius:0,tension:.3},"
        "{label:'Base théorique',data:new Array(st.tl.length).fill(bv),borderColor:'rgba(255,61,94,.55)',borderWidth:1.5,pointRadius:0,fill:false,borderDash:[5,3]}"
        "]},options:cd5});"
        # Chart 6
        "const cd6=JSON.parse(JSON.stringify(CD));"
        "cd6.scales.y.min=0;cd6.scales.y.max=100;"
        "cd6.scales.y.ticks={color:'#5c6b8c',font:{size:9},callback:v=>v+'%'};"
        "mk('c6',{type:'bar',data:{labels:st.ml,datasets:["
        "{label:'% ticks haussiers',data:st.mv,"
        "backgroundColor:st.mv.map(v=>v>=50?'rgba(0,255,157,.65)':'rgba(255,61,94,.65)'),borderRadius:3},"
        "{label:'Biais naturel (48%)',data:new Array(st.ml.length).fill(48),"
        "type:'line',borderColor:'rgba(255,176,32,.8)',borderWidth:2,pointRadius:0,fill:false,borderDash:[5,3]}"
        "]},options:cd6});"
        "}finally{"
    ...
    return Response(H + JS_HEAD + data_js + JS_TAIL, content_type="text/html; charset=utf-8")
