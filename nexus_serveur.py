...
<title>Nexus — Panel Admin</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js"></script>
<style>
*{box-sizing:border-box;margin:0;padding:0}
:root{--bg:#070910;--panel:#0d1020;--card:#111525;--border:#1a2240;--cyan:#00e5ff;--purple:#a06bff;--green:#00ff9d;--gold:#ffb020;--red:#ff4757;--pink:#ff5db1;--text:#dce8ff;--muted:#3d4f7a}
body{background:var(--bg);color:var(--text);font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh;overflow-x:hidden}
::-webkit-scrollbar{width:4px;height:4px}
::-webkit-scrollbar-thumb{background:var(--border);border-radius:4px}
/* Login */
#loginWrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:var(--bg);z-index:200}
.login-box{width:100%;max-width:380px;padding:0 20px}
.login-logo{text-align:center;margin-bottom:32px}
.login-logo-text{font-size:36px;font-weight:900;letter-spacing:4px;background:linear-gradient(90deg,var(--cyan),var(--purple));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.login-logo-sub{font-size:11px;color:var(--muted);letter-spacing:3px;text-transform:uppercase;margin-top:4px}
.login-field{width:100%;background:var(--card);border:1px solid var(--border);border-radius:12px;padding:14px 18px;color:var(--text);font-size:15px;outline:none;margin-bottom:14px;transition:.2s}
.login-field:focus{border-color:var(--cyan);box-shadow:0 0 0 3px rgba(0,229,255,.08)}
.login-btn{width:100%;padding:14px;background:linear-gradient(90deg,rgba(0,229,255,.15),rgba(160,107,255,.15));border:1px solid;border-image:linear-gradient(90deg,var(--cyan),var(--purple)) 1;border-radius:12px;color:var(--text);font-size:15px;font-weight:700;cursor:pointer;letter-spacing:1px;transition:.2s}
.login-btn:hover{background:linear-gradient(90deg,rgba(0,229,255,.25),rgba(160,107,255,.25))}
.login-msg{text-align:center;font-size:12px;min-height:18px;margin-top:8px}
/* Main app */
#app{display:none;min-height:100vh}
/* Top bar */
.topbar{background:var(--panel);border-bottom:1px solid var(--border);padding:0 20px;display:flex;align-items:center;gap:16px;position:sticky;top:0;z-index:100;height:52px}
.tb-logo{font-size:16px;font-weight:900;letter-spacing:2px;background:linear-gradient(90deg,var(--cyan),var(--purple));-webkit-background-clip:text;-webkit-text-fill-color:transparent;white-space:nowrap}
.tb-status{font-size:10px;padding:3px 9px;border-radius:20px;background:rgba(0,255,157,.1);color:var(--green);border:1px solid rgba(0,255,157,.2)}
.tb-right{margin-left:auto;display:flex;align-items:center;gap:8px}
.tb-btn{background:transparent;border:1px solid var(--border);border-radius:8px;padding:5px 12px;color:var(--muted);font-size:11px;cursor:pointer;transition:.15s}
.tb-btn:hover{border-color:var(--cyan);color:var(--cyan)}
/* Tabs */
.tabs{background:var(--panel);border-bottom:1px solid var(--border);display:flex;overflow-x:auto;padding:0 16px;gap:2px}
.tab{padding:12px 18px;font-size:12px;font-weight:700;color:var(--muted);cursor:pointer;border-bottom:2px solid transparent;white-space:nowrap;transition:.15s;letter-spacing:.5px}
...
.card-title{font-size:12px;font-weight:700;color:var(--cyan);letter-spacing:1px;text-transform:uppercase;margin-bottom:14px;display:flex;align-items:center;gap:8px}
/* Grid */
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px}
@media(max-width:700px){.grid2,.grid3{grid-template-columns:1fr}}
/* Stat chips */
.stat{background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:14px 18px;text-align:center}
.stat-n{font-size:26px;font-weight:900;margin-bottom:4px}
.stat-l{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px}
/* Table */
.tbl{width:100%;border-collapse:collapse;font-size:12px}
.tbl th{color:var(--muted);padding:8px 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);white-space:nowrap}
.tbl td{padding:9px 12px;border-bottom:1px solid rgba(255,255,255,.03);vertical-align:middle;word-break:break-word}
...
.badge-admin{background:rgba(255,93,177,.12);color:var(--pink);border:1px solid rgba(255,93,177,.2)}
.badge-mod{background:rgba(0,255,157,.1);color:var(--green);border:1px solid rgba(0,255,157,.2)}
.badge-vip{background:rgba(255,176,32,.1);color:var(--gold);border:1px solid rgba(255,176,32,.2)}
.badge-user{background:rgba(160,107,255,.1);color:var(--purple);border:1px solid rgba(160,107,255,.2)}
/* Buttons */
.btn{padding:7px 14px;border-radius:9px;border:1px solid;cursor:pointer;font-size:12px;font-weight:700;background:transparent;transition:.15s}
.btn-cyan{border-color:var(--cyan);color:var(--cyan)}.btn-cyan:hover{background:rgba(0,229,255,.1)}
.btn-green{border-color:var(--green);color:var(--green)}.btn-green:hover{background:rgba(0,255,157,.1)}
.btn-red{border-color:var(--red);color:var(--red)}.btn-red:hover{background:rgba(255,71,87,.1)}
.btn-gold{border-color:var(--gold);color:var(--gold)}.btn-gold:hover{background:rgba(255,176,32,.1)}
.btn-purple{border-color:var(--purple);color:var(--purple)}.btn-purple:hover{background:rgba(160,107,255,.1)}
.btn-muted{border-color:var(--border);color:var(--muted)}.btn-muted:hover{border-color:var(--text);color:var(--text)}
/* Search */
.fi{background:var(--panel);border:1px solid var(--border);border-radius:9px;padding:9px 13px;color:var(--text);font-size:12px;outline:none;transition:.15s}
.fi:focus{border-color:var(--cyan)}.fi::placeholder{color:var(--muted)}
/* User row */
.user-row{display:flex;align-items:center;gap:12px;padding:12px;border-radius:12px;border:1px solid transparent;cursor:pointer;transition:.15s;margin-bottom:6px}
.user-row:hover{background:rgba(255,255,255,.03);border-color:var(--border)}
.user-av{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px;background:rgba(160,107,255,.1);border:1px solid rgba(160,107,255,.2);flex-shrink:0}
...
/* Connection card */
.conn-card{background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:10px;position:relative}
.conn-hdr{display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap}
.conn-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px}
.conn-field{background:rgba(255,255,255,.03);border-radius:8px;padding:8px 10px}
.conn-field-label{font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px}
.conn-field-val{font-size:11px;font-weight:600;word-break:break-all}
...
.tl-dot{position:absolute;left:-20px;top:4px;width:8px;height:8px;border-radius:50%;background:var(--cyan);border:2px solid var(--bg)}
.tl-time{font-size:10px;color:var(--muted);margin-bottom:2px}
.tl-content{font-size:11px;background:rgba(255,255,255,.03);border-radius:8px;padding:6px 10px}
/* Log rows */
.log-row{display:grid;grid-template-columns:60px 50px 1fr 80px 60px;gap:8px;padding:6px 10px;border-bottom:1px solid rgba(255,255,255,.03);font-size:11px;font-family:monospace;align-items:center}
.log-row:hover{background:rgba(255,255,255,.02)}
.m-get{color:var(--cyan)}.m-post{color:var(--green)}.m-del{color:var(--red)}
.s-2{color:var(--green)}.s-4{color:var(--gold)}.s-5{color:var(--red)}
/* Modal */
.modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:300;display:flex;align-items:center;justify-content:center;padding:16px}
.modal-box{background:var(--card);border:1px solid var(--border);border-radius:18px;max-width:720px;width:100%;max-height:90vh;overflow:auto}
.modal-hdr{padding:18px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px;position:sticky;top:0;background:var(--card);z-index:1}
.modal-body{padding:20px}
.modal-tabs{display:flex;gap:4px;padding:0 20px;border-bottom:1px solid var(--border);background:var(--card)}
.modal-tab{padding:10px 16px;font-size:11px;font-weight:700;color:var(--muted);cursor:pointer;border-bottom:2px solid transparent}
.modal-tab.on{color:var(--cyan);border-bottom-color:var(--cyan)}
/* History item */
.hist-item{display:grid;grid-template-columns:1fr 120px;gap:8px;padding:8px 12px;border-bottom:1px solid rgba(255,255,255,.03);font-size:11px}
.hist-item:hover{background:rgba(255,255,255,.02)}
.hist-query{font-weight:600;color:var(--text);margin-bottom:3px}
.hist-url{color:var(--muted);font-family:monospace;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
/* Chart containers */
.chart-wrap{position:relative;height:240px}
/* Messages */
.msg-item{padding:12px;border:1px solid var(--border);border-radius:10px;margin-bottom:8px}
.msg-to{font-size:11px;color:var(--muted);margin-bottom:4px}
.msg-text{font-size:13px}
.msg-ts{font-size:10px;color:var(--muted);margin-top:4px}
/* Print */
@media print{
  .topbar,.tabs,.modal-overlay,.btn,.tb-btn,#loginWrap{display:none!important}
  .conn-actions{display:none!important}
  .card{border:1px solid #ccc!important;background:white!important;color:black!important}
  .conn-field{background:#f5f5f5!important}
  .conn-field-label{color:#666!important}
  .conn-field-val{color:black!important}
}
/* Import overlay */
.import-zone{border:2px dashed var(--border);border-radius:12px;padding:32px;text-align:center;cursor:pointer;transition:.2s}
.import-zone:hover,.import-zone.drag{border-color:var(--cyan);background:rgba(0,229,255,.04)}
textarea.fi{resize:vertical;min-height:100px;font-family:monospace}
</style>
</head>
<body>
...
<div id="loginWrap">
  <div class="login-box">
    <div class="login-logo">
      ...
      <div class="login-logo-sub">Panel Administration</div>
    </div>
    <input class="login-field" id="mkInp" type="password" placeholder="Clé maître" autocomplete="off">
...
<div id="app">
  <div class="topbar">
    <span class="tb-logo">◈ NEXUS ADMIN</span>
    <span class="tb-status" id="tbStatus">● En ligne</span>
    <span style="font-size:11px;color:var(--muted)" id="tbClock"></span>
    <div class="tb-right">
      <button class="tb-btn" onclick="doRefresh()">🔄 Actualiser</button>
      <button class="tb-btn" onclick="doLogout()">Déconnexion</button>
    </div>
  </div>
  <div class="tabs">
    <div class="tab on" onclick="showPage('accounts',this)">👥 Comptes</div>
    <div class="tab" onclick="showPage('piratage',this)">📡 Piratage</div>
    <div class="tab" onclick="showPage('history',this)">🌐 Historique</div>
    <div class="tab" onclick="showPage('stats',this)">📊 Statistiques</div>
    <div class="tab" onclick="showPage('messages',this)">💬 Messages</div>
    <div class="tab" onclick="showPage('monitor',this)">🖥️ Monitor</div>
  </div>

  <!-- PAGE: COMPTES -->
  <div class="page on" id="page-accounts">
    <div class="card">
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px">
        <input class="fi" id="accSearch" placeholder="🔍 Rechercher un compte…" oninput="renderAccounts()" style="flex:1;min-width:200px">
        <select class="fi" id="accRole" onchange="renderAccounts()">
        ...
        <label class="fi" style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" id="showHidden" onchange="renderAccounts()"> Masqués</label>
      </div>
      <div id="accList"></div>
    </div>
    <div class="card">
      <div class="card-title">➕ Nouveau compte</div>
      <div style="display:grid;grid-template-columns:1fr 1fr auto auto;gap:10px;flex-wrap:wrap" class="grid2" id="createRow">
        <input class="fi" id="nuUser" placeholder="Nom d'utilisateur">
        <input class="fi" id="nuPass" type="text" placeholder="Mot de passe">
        ...
        <button class="btn btn-cyan" onclick="createAccount()">Créer</button>
      </div>
      <div id="createMsg" style="margin-top:8px;font-size:12px"></div>
    </div>
  </div>

  <!-- PAGE: PIRATAGE -->
  <div class="page" id="page-piratage">
    <div class="card">
      <div class="card-title">📡 Connexions — Toutes les données</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        <button class="btn btn-cyan" onclick="loadAllConns()">🔄 Charger toutes les connexions</button>
        <button class="btn btn-gold" onclick="exportConnsJSON()">⬇ Exporter JSON</button>
        <label class="btn btn-purple" style="cursor:pointer">📥 Importer JSON<input type="file" accept=".json" style="display:none" onchange="importConnsFile(this)"></label>
        <select class="fi" id="connFilterUser" onchange="renderConns()"><option value="">Tous les utilisateurs</option></select>
        <select class="fi" id="connFilterApp" onchange="renderConns()"><option value="">Toutes les apps</option></select>
        <input class="fi" id="connFilterCountry" placeholder="🌍 Pays…" oninput="renderConns()">
        <button class="btn btn-muted" onclick="clearConnFilters()">✕ Réinitialiser</button>
      </div>
      <div id="connCount" style="font-size:11px;color:var(--muted);margin-bottom:12px"></div>
      <div id="connList"></div>
    </div>
  </div>
  ...
  <div class="page" id="page-history">
    <div class="card">
      <div class="card-title">🌐 Historique de navigation</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        <input class="fi" id="histSearch" placeholder="🔍 Rechercher…" oninput="renderHistory()">
        <select class="fi" id="histUser" onchange="renderHistory()"><option value="">Tous les utilisateurs</option></select>
        <select class="fi" id="histApp" onchange="renderHistory()"><option value="">Toutes les apps</option></select>
      </div>
      <div id="histList"></div>
    </div>
  </div>
    ...
    <div class="grid2">
      <div class="card">
        ...
        <div style="margin-bottom:10px">
          <select class="fi" id="msgTarget" style="width:100%;margin-bottom:10px">
            <option value="__all__">📢 Tous les utilisateurs</option>
          </select>
          <textarea class="fi" id="msgText" placeholder="Votre message aux utilisateurs Nexus…" style="width:100%;resize:vertical;min-height:100px;margin-bottom:10px"></textarea>
          <button class="btn btn-cyan" onclick="sendMessage()" style="width:100%">📨 Envoyer</button>
          <div id="msgResult" style="margin-top:8px;font-size:12px"></div>
        </div>
      </div>
      <div class="card">
        <div class="card-title">📬 Messages envoyés</div>
        <div id="sentMessages" style="max-height:400px;overflow-y:auto"></div>
      </div>
    </div>
  </div>
  ...
  <div class="page" id="page-monitor">
    <div class="grid2">
      <div class="card">
        <div class="card-title">⚡ Statut serveur</div>
        <div class="grid2">
          <div class="stat"><div class="stat-n" id="monUptime" style="color:var(--green)">—</div><div class="stat-l">Uptime</div></div>
          <div class="stat"><div class="stat-n" id="monReqs" style="color:var(--cyan)">—</div><div class="stat-l">Requêtes (mémoire)</div></div>
          <div class="stat"><div class="stat-n" id="monAvgMs" style="color:var(--gold)">—</div><div class="stat-l">Temps moyen (ms)</div></div>
          <div class="stat"><div class="stat-n" id="monErrors" style="color:var(--red)">—</div><div class="stat-l">Erreurs 4xx/5xx</div></div>
        </div>
      </div>
      <div class="card">
    ...
    <div class="card">
      <div class="card-title" style="justify-content:space-between">
        🔴 Logs en direct
        <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--muted);font-weight:400"><input type="checkbox" id="autoRefreshLog" checked> Auto-refresh</label>
      </div>
      <div style="display:grid;grid-template-columns:60px 50px 1fr 80px 60px;gap:8px;padding:6px 10px;border-bottom:1px solid var(--border);font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">
        <span>Méthode</span><span>Status</span><span>Chemin</span><span>IP</span><span>ms</span>
      </div>
      <div id="logList" style="max-height:400px;overflow-y:auto;font-family:monospace"></div>
    </div>
  </div>
</div>

<!-- MODAL -->
<div id="modalWrap" style="display:none" onclick="if(event.target===this)closeModal()">
  <div class="modal-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:300;display:flex;align-items:center;justify-content:center;padding:16px">
    <div class="modal-box" id="modalBox"></div>
  </div>
</div>

<script>
var KEY='',USERS=[],ALL_CONNS=[],ALL_HIST=[],_sentMsgs=[],_charts={},_monChart=null;
var _refreshInt=null,_monInt=null,_clockInt=null;

function esc(s){return(s+'').replace(/[&<>"]/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];})}
function jsq(s){return(s+'').replace(/\\\\/g,'\\\\\\\\').replace(/'/g,"\\\\'")}
function fmtDate(ts){if(!ts)return '—';try{return new Date(typeof ts==='number'&&ts<1e12?ts*1000:ts).toLocaleString('fr-FR');}catch(e){return String(ts);}}
function fmtUptime(s){var d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60);return(d?d+'j ':'')+(h?h+'h ':'')+m+'min';}
function roleBadge(r){var map={admin:'<span class="badge badge-admin">👑 Admin</span>',moderator:'<span class="badge badge-mod">🛡️ Modérateur</span>',vip:'<span class="badge badge-vip">⭐ VIP</span>',user:'<span class="badge badge-user">👤 Utilisateur</span>'};return map[r]||map.user;}
function roleIcon(r){return{admin:'👑',moderator:'🛡️',vip:'⭐',user:'👤'}[r]||'👤';}
...
async function apiGet(path){
  try{var r=await fetch(path+'?k='+encodeURIComponent(KEY));return await r.json();}
  catch(e){return{ok:false,error:String(e)};}
}

// ── LOGIN ──
document.getElementById('mkInp').addEventListener('keydown',function(e){if(e.key==='Enter')doLogin();});
async function doLogin(){
  KEY=document.getElementById('mkInp').value.trim();
  ...
  if(r&&r.ok){
    ...
    renderAccounts();
    startClock();
    _refreshInt=setInterval(function(){silentRefresh();},8000);
    _monInt=setInterval(function(){if(document.getElementById('page-monitor').classList.contains('on'))loadMonitor();},4000);
  }else{
    msg.style.color='var(--red)';msg.textContent='❌ Clé maître refusée';KEY='';
  }
}
function doLogout(){
  KEY='';USERS=[];ALL_CONNS=[];ALL_HIST=[];
  clearInterval(_refreshInt);clearInterval(_monInt);clearInterval(_clockInt);
  document.getElementById('loginWrap').style.display='flex';
  document.getElementById('app').style.display='none';
  document.getElementById('mkInp').value='';
...
async function silentRefresh(){
  ...
  if(r&&r.ok){
    USERS=r.users||[];
    document.getElementById('tbStatus').textContent='● En ligne';
    document.getElementById('tbStatus').style.color='var(--green)';
    if(document.getElementById('page-accounts').classList.contains('on'))renderAccounts();
  }else{
    document.getElementById('tbStatus').textContent='● Reconnexion…';
    document.getElementById('tbStatus').style.color='var(--gold)';
  }
}
async function doRefresh(){
  await silentRefresh();
  if(document.getElementById('page-piratage').classList.contains('on'))await loadAllConns();
  if(document.getElementById('page-stats').classList.contains('on'))await loadStats();
  if(document.getElementById('page-monitor').classList.contains('on'))await loadMonitor();
}
function startClock(){
  _clockInt=setInterval(function(){document.getElementById('tbClock').textContent=new Date().toLocaleTimeString('fr-FR');},1000);
}
...
function showPage(id,el){
  ...
  if(id==='history')buildHistoryFromConns();
}

// ── COMPTES ──
function renderAccounts(){
  var q=(document.getElementById('accSearch').value||'').toLowerCase();
  var rf=(document.getElementById('accRole').value||'');
  var sh=document.getElementById('showHidden').checked;
  var list=document.getElementById('accList');
  var filtered=USERS.filter(function(u){
    if(u.hidden&&!sh)return false;
    if(rf&&u.role!==rf)return false;
    if(q&&!u.username.toLowerCase().includes(q)&&!(u.nickname||'').toLowerCase().includes(q))return false;
    return true;
  });
  if(!filtered.length){list.innerHTML='<div style="text-align:center;color:var(--muted);padding:30px;font-size:12px">Aucun compte trouvé</div>';return;}
  list.innerHTML=filtered.map(function(u){
    var nick=u.nickname?' <span style="color:var(--muted);font-size:11px">«'+esc(u.nickname)+'»</span>':'';
    var lastSeen=u.last_login?'Vu '+fmtDate(u.last_login)+' · '+esc(u.last_ip||''):'Jamais connecté';
    return '<div class="user-row" onclick="openUserModal(\''+jsq(u.username)+'\')">'+
      '<div class="user-av">'+roleIcon(u.role)+'</div>'+
      '<div class="user-info">'+
        '<div class="user-name">'+esc(u.username)+nick+'</div>'+
        '<div class="user-meta">'+lastSeen+' · '+u.history+' pages nav.</div>'+
      '</div>'+
      roleBadge(u.role)+
      '</div>';
  }).join('');
...
async function openUserModal(name){
  ...
  document.getElementById('modalBox').innerHTML='<div class="modal-hdr"><span style="font-size:20px">⏳</span><span>Chargement…</span></div>';
  var res=await api('/admin/get',{target:name});
  if(!res.ok){document.getElementById('modalBox').innerHTML='<div style="padding:20px;color:var(--red)">Erreur: '+esc(res.error||'')+'</div>';return;}
  var d=res.data||{};
  var nx=d.nx2098||{};var nxc=d.nxcoin||{};var hist=Array.isArray(d.history)?d.history:[];
  var conns=Array.isArray(d.connections)?d.connections:[];
  var role=res.role||'user';
  // Sort history: most recent first
  hist=hist.slice().sort(function(a,b){return(b.time||b.ts||0)-(a.time||a.ts||0);});
  conns=conns.slice().sort(function(a,b){return(b.ts||0)-(a.ts||0);});
  var tabId='utab-prof';
  function render(tid){
    var profActive=tid==='utab-prof'?'on':'';
    var histActive=tid==='utab-hist'?'on':'';
    var connActive=tid==='utab-conn'?'on':'';
    var actActive=tid==='utab-act'?'on':'';
    var html='<div class="modal-hdr">'+
      '<span style="font-size:24px">'+roleIcon(role)+'</span>'+
      '<div><div style="font-size:16px;font-weight:700">'+esc(name)+'</div><div style="font-size:11px;color:var(--muted)">'+roleBadge(role)+'</div></div>'+
      '<button onclick="closeModal()" style="margin-left:auto;background:transparent;border:none;color:var(--muted);font-size:20px;cursor:pointer">✕</button>'+
    '</div>'+
    '<div class="modal-tabs">'+
      '<div class="modal-tab '+profActive+'" onclick="switchModalTab(\'utab-prof\',\''+jsq(name)+'\')">👤 Profil</div>'+
      '<div class="modal-tab '+histActive+'" onclick="switchModalTab(\'utab-hist\',\''+jsq(name)+'\')">🌐 Historique ('+hist.length+')</div>'+
      '<div class="modal-tab '+connActive+'" onclick="switchModalTab(\'utab-conn\',\''+jsq(name)+'\')">📡 Connexions ('+conns.length+')</div>'+
      '<div class="modal-tab '+actActive+'" onclick="switchModalTab(\'utab-act\',\''+jsq(name)+'\')">⚙️ Actions</div>'+
    '</div>'+
    '<div class="modal-body">';
    // PROFIL
    html+='<div id="utab-prof" '+(tid==='utab-prof'?'':'style="display:none"')+'>'+
      '<div class="grid2" style="margin-bottom:14px">'+
        '<div class="stat"><div class="stat-n" style="color:var(--gold)">'+(nx.rewards||0)+'</div><div class="stat-l">Rewards</div></div>'+
        '<div class="stat"><div class="stat-n" style="color:var(--cyan)">'+(nxc.nxc||0)+' NXC</div><div class="stat-l">NexusCoin</div></div>'+
      '</div>';
    if(nxc.stake){html+='<div style="background:rgba(0,255,157,.06);border:1px solid rgba(0,255,157,.2);border-radius:10px;padding:10px 14px;margin-bottom:12px;font-size:12px">🔒 Staking: <b>'+nxc.stake.amt+' NXC</b> @ '+nxc.stake.apy.toFixed(1)+'% APY — fin '+fmtDate(nxc.stake.end)+'</div>';}
    html+='<table class="tbl"><tbody>'+
      '<tr><td style="color:var(--muted)">Dernière connexion</td><td>'+fmtDate(res.last_login)+'</td></tr>'+
      '<tr><td style="color:var(--muted)">Dernière IP</td><td style="font-family:monospace">'+esc(res.last_ip||'—')+'</td></tr>'+
      '<tr><td style="color:var(--muted)">Pages naviguées</td><td>'+hist.length+'</td></tr>'+
      '<tr><td style="color:var(--muted)">Connexions appareils</td><td>'+conns.length+'</td></tr>'+
    '</tbody></table></div>';
    // HISTORIQUE NAV
    html+='<div id="utab-hist" '+(tid==='utab-hist'?'':'style="display:none"')+' style="max-height:480px;overflow-y:auto">';
    if(!hist.length){html+='<div style="text-align:center;color:var(--muted);padding:30px;font-size:12px">Aucun historique</div>';}
    else{
      html+='<div style="font-size:10px;color:var(--muted);padding:6px 12px;text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border)">'+hist.length+' entrée(s)</div>';
      hist.slice(0,200).forEach(function(h){
        var q=h.label||h.query||h.title||'';
        var url=h.url||'';
        var app=h.app||'';
        html+='<div class="hist-item">'+
          '<div>'+
            '<div class="hist-query">'+esc(q)+'</div>'+
            (url?'<div class="hist-url">'+esc(url)+'</div>':'')+
            (app?'<span style="font-size:9px;background:rgba(160,107,255,.1);color:var(--purple);border-radius:10px;padding:2px 7px">'+esc(app)+'</span>':'')+
          '</div>'+
          '<div style="text-align:right;font-size:10px;color:var(--muted)">'+fmtDate(h.time||h.ts)+'</div>'+
        '</div>';
      });
    }
    html+='</div>';
    // CONNEXIONS (timeline)
    html+='<div id="utab-conn" '+(tid==='utab-conn'?'':'style="display:none"')+' style="max-height:480px;overflow-y:auto">';
    if(!conns.length){html+='<div style="text-align:center;color:var(--muted);padding:30px;font-size:12px">Aucune connexion enregistrée</div>';}
    else{
      html+='<div class="timeline">';
      conns.slice(0,100).forEach(function(c){
        html+='<div class="tl-item"><div class="tl-dot"></div>'+
          '<div class="tl-time">'+fmtDate(c.ts||c.tsStr)+' · <span style="color:var(--green)">'+esc(c.app||'Nexus')+'</span></div>'+
          '<div class="tl-content">'+esc((c.brand||'')+' '+(c.model||'')).trim()+' · '+esc(c.platform||c.os||'?')+' · <span style="font-family:monospace">'+esc(c.ip||'?')+'</span> '+esc(c.country||'')+'</div>'+
        '</div>';
      });
      html+='</div>';
    }
    html+='</div>';
    // ACTIONS
    html+='<div id="utab-act" '+(tid==='utab-act'?'':'style="display:none"')+'>'+
      '<div style="display:grid;gap:10px">'+
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
        '<span style="font-size:12px;color:var(--muted);flex:1">Rôle</span>'+
        '<select class="fi" id="actRole"><option value="user">👤 Utilisateur</option><option value="moderator">🛡️ Modérateur</option><option value="vip">⭐ VIP</option><option value="admin">👑 Admin</option></select>'+
        '<button class="btn btn-cyan" onclick="setRole(\''+jsq(name)+'\')">Appliquer</button>'+
      '</div>'+
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
        '<span style="font-size:12px;color:var(--muted);flex:1">Rewards</span>'+
        '<input class="fi" id="actRewards" type="number" placeholder="Montant" style="width:120px">'+
        '<button class="btn btn-gold" onclick="giveRewards(\''+jsq(name)+'\')">Donner</button>'+
      '</div>'+
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
        '<span style="font-size:12px;color:var(--muted);flex:1">Renommer</span>'+
        '<input class="fi" id="actRename" placeholder="Nouveau nom" style="width:160px">'+
        '<button class="btn btn-purple" onclick="renameUser(\''+jsq(name)+'\')">Renommer</button>'+
      '</div>'+
      '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
        '<span style="font-size:12px;color:var(--muted);flex:1">Surnom</span>'+
        '<input class="fi" id="actNick" placeholder="Surnom affiché" style="width:160px">'+
        '<button class="btn btn-purple" onclick="setNick(\''+jsq(name)+'\')">Définir</button>'+
      '</div>'+
      '<div style="display:flex;gap:8px;flex-wrap:wrap;border-top:1px solid var(--border);padding-top:12px;margin-top:4px">'+
        '<button class="btn btn-muted" onclick="toggleHide(\''+jsq(name)+'\','+(res.hidden?'false':'true')+')">'+
          (res.hidden?'👁️ Afficher':'🙈 Masquer')+
        '</button>'+
        '<button class="btn btn-red" onclick="deleteUser(\''+jsq(name)+'\')">🗑️ Supprimer</button>'+
      '</div></div>';
    html+='<div id="actMsg" style="margin-top:10px;font-size:12px"></div></div>';
    html+='</div>';
    document.getElementById('modalBox').innerHTML=html;
    // Set current role in selector
    var sel=document.getElementById('actRole');if(sel)sel.value=role;
  }
  window._modalUserData={name:name,res:res,d:d,hist:hist,conns:conns,role:role};
  render(tabId);
  window._renderModalUser=render;
}
function switchModalTab(tid,name){if(window._renderModalUser)window._renderModalUser(tid);}
function closeModal(){document.getElementById('modalWrap').style.display='none';}

async function actMsg(txt,ok){var el=document.getElementById('actMsg');if(el){el.style.color=ok?'var(--green)':'var(--red)';el.textContent=txt;}}
async function setRole(name){var r=document.getElementById('actRole').value;var res=await api('/admin/set-role',{target:name,role:r});actMsg(res.ok?'Rôle mis à jour ✓':('Erreur: '+(res.error||'')),res.ok);if(res.ok)silentRefresh();}
async function giveRewards(name){var a=parseFloat(document.getElementById('actRewards').value);if(!a)return;var res=await api('/admin/give-rewards',{target:name,amount:a});actMsg(res.ok?'Rewards donnés ✓':('Erreur: '+(res.error||'')),res.ok);}
async function renameUser(name){var n=document.getElementById('actRename').value.trim();if(!n)return;var res=await api('/admin/rename',{target:name,new_username:n});actMsg(res.ok?'Renommé ✓':('Erreur: '+(res.error||'')),res.ok);if(res.ok){closeModal();silentRefresh();}}
async function setNick(name){var n=document.getElementById('actNick').value;var res=await api('/admin/nickname',{target:name,nickname:n});actMsg(res.ok?'Surnom défini ✓':('Erreur: '+(res.error||'')),res.ok);}
async function toggleHide(name,hide){var res=await api('/admin/hide',{target:name,hidden:hide});actMsg(res.ok?'OK':('Erreur: '+(res.error||'')),res.ok);if(res.ok)silentRefresh();}
async function deleteUser(name){if(!confirm('Supprimer définitivement « '+name+' » ?'))return;var res=await api('/admin/purge',{target:name});if(res.ok){closeModal();silentRefresh();}else actMsg('Erreur: '+(res.error||''),false);}
async function createAccount(){
  var u=document.getElementById('nuUser').value.trim();
  var p=document.getElementById('nuPass').value;
  var r=document.getElementById('nuRole').value;
  var msg=document.getElementById('createMsg');
  if(!u||!p){msg.style.color='var(--red)';msg.textContent='Nom et mot de passe requis';return;}
  var res=await api('/admin/create',{new_username:u,new_password:p,role:r});
  if(res.ok){msg.style.color='var(--green)';msg.textContent='✓ Compte créé';document.getElementById('nuUser').value='';document.getElementById('nuPass').value='';silentRefresh();}
  else{msg.style.color='var(--red)';msg.textContent='Erreur: '+(res.error||'');}
}

// ── PIRATAGE ──
var _allConnsLoaded=false;
async function loadAllConns(){
  document.getElementById('connCount').textContent='⏳ Chargement en cours…';
  document.getElementById('connList').innerHTML='';
  ALL_CONNS=[];
  var results=await Promise.all(USERS.map(async function(u){
    try{var r=await api('/admin/get',{target:u.username});
      if(r.ok){
        var conns=((r.data||{}).connections)||[];
        conns.forEach(function(c){c._user=c._user||u.username;});
        return conns;
      }
    }catch(e){}return[];
  }));
  results.forEach(function(c){ALL_CONNS=ALL_CONNS.concat(c);});
  ALL_CONNS.sort(function(a,b){return(b.ts||0)-(a.ts||0);});
  _allConnsLoaded=true;
  // Populate filter dropdowns
  var users=[...new Set(ALL_CONNS.map(function(c){return c._user||c.user||'?';}))].sort();
  var apps=[...new Set(ALL_CONNS.map(function(c){return c.app||'Nexus';}))].sort();
  var uSel=document.getElementById('connFilterUser');
  uSel.innerHTML='<option value="">Tous les utilisateurs ('+users.length+')</option>'+users.map(function(u){return'<option value="'+esc(u)+'">'+esc(u)+'</option>';}).join('');
  var aSel=document.getElementById('connFilterApp');
  aSel.innerHTML='<option value="">Toutes les apps</option>'+apps.map(function(a){return'<option value="'+esc(a)+'">'+esc(a)+'</option>';}).join('');
  buildHistoryFromConns();
  renderConns();
}
function clearConnFilters(){
  document.getElementById('connFilterUser').value='';
  document.getElementById('connFilterApp').value='';
  document.getElementById('connFilterCountry').value='';
  renderConns();
}
function renderConns(){
  if(!_allConnsLoaded){document.getElementById('connCount').textContent='Clique sur "Charger toutes les connexions" pour commencer.';document.getElementById('connList').innerHTML='';return;}
  var fu=document.getElementById('connFilterUser').value;
  var fa=document.getElementById('connFilterApp').value;
  var fc=(document.getElementById('connFilterCountry').value||'').toLowerCase();
  var filtered=ALL_CONNS.filter(function(c){
    if(fu&&(c._user||c.user||'')!==fu)return false;
    if(fa&&(c.app||'Nexus')!==fa)return false;
    if(fc&&!(c.country||'').toLowerCase().includes(fc))return false;
    return true;
  });
  document.getElementById('connCount').textContent=filtered.length+' connexion(s) affichée(s) sur '+ALL_CONNS.length+' total';
  document.getElementById('connList').innerHTML=filtered.slice(0,200).map(function(c,i){
    return connCard(c,i);
  }).join('');
}
function connCard(c,i){
  var idx=i||0;
  var user=c._user||c.user||'?';
  var fields=[
    ['👤 Utilisateur',user],['📲 App Nexus',c.app||'Nexus'],['📅 Date',fmtDate(c.ts||c.tsStr)],
    ['🌐 IP',c.ip||'?'],['🌍 Pays',c.country||'?'],['🏙️ Ville',c.city||'?'],
    ['📱 Plateforme',c.platform||'?'],['💻 OS',c.os||'?'],['🌐 Navigateur',c.browser||'?'],
    ['📱 Marque',c.brand||'?'],['📱 Modèle',c.model||'?'],['🖥️ Écran',c.screen||'?'],
    ['🎮 GPU',c.gpu||'?'],['🧠 RAM',c.ram?c.ram+'GB':'?'],['⚙️ Cores',c.cores||'?'],
    ['🔋 Batterie',c.batt!==undefined&&c.batt!==null?c.batt+'%':'?'],['📶 Réseau',c.net||c.network||'?'],
  ];
  var html='<div class="conn-card" id="cc'+idx+'">'+
    '<div class="conn-hdr">'+
      '<span style="font-size:20px">📡</span>'+
      '<div>'+
        '<div style="font-weight:700;font-size:13px">'+esc(user)+'</div>'+
        '<div style="font-size:11px;color:var(--muted)">'+fmtDate(c.ts||c.tsStr)+'</div>'+
      '</div>'+
      '<span style="background:rgba(0,229,255,.1);color:var(--cyan);border-radius:8px;padding:3px 10px;font-size:10px;font-weight:700">'+esc(c.app||'Nexus')+'</span>'+
      '<span style="background:rgba(160,107,255,.1);color:var(--purple);border-radius:8px;padding:3px 10px;font-size:10px">'+esc(c.country||'?')+'</span>'+
    '</div>'+
    '<div class="conn-grid">';
  fields.forEach(function(f){
    if(!f[1]||f[1]==='?')return;
    html+='<div class="conn-field"><div class="conn-field-label">'+f[0]+'</div><div class="conn-field-val">'+esc(f[1])+'</div></div>';
  });
  html+='</div>'+
  '<div class="conn-actions">'+
    '<button class="btn btn-muted" style="font-size:11px" onclick="printConn('+idx+')">🖨️ Imprimer</button>'+
    '<button class="btn btn-gold" style="font-size:11px" onclick="exportSingleConn('+idx+')">⬇ JSON</button>'+
  '</div></div>';
  return html;
}
window._renderedConns=[];
function exportConnsJSON(){
  var blob=new Blob([JSON.stringify(ALL_CONNS,null,2)],{type:'application/json'});
  var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='nexus_connexions_'+Date.now()+'.json';a.click();
}
function exportSingleConn(i){
  var c=ALL_CONNS[i];if(!c)return;
  var blob=new Blob([JSON.stringify(c,null,2)],{type:'application/json'});
  var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='conn_'+Date.now()+'.json';a.click();
}
function printConn(i){
  var el=document.getElementById('cc'+i);if(!el)return;
  var w=window.open('','_print','width=800,height=600');
  w.document.write('<html><head><title>Connexion — Nexus</title><style>body{font-family:Segoe UI,sans-serif;padding:20px}.conn-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.conn-field{border:1px solid #ccc;border-radius:6px;padding:8px}.conn-field-label{font-size:10px;color:#888;text-transform:uppercase;margin-bottom:3px}.conn-field-val{font-size:12px;font-weight:600}</style></head><body>'+el.innerHTML+'</body></html>');
  w.document.close();w.print();w.close();
}
async function importConnsFile(input){
  var f=input.files[0];if(!f)return;
  var reader=new FileReader();
  reader.onload=async function(e){
    try{
      var data=JSON.parse(e.target.result);
      var conns=Array.isArray(data)?data:(data.connections||[]);
      if(!conns.length){alert('Aucune connexion trouvée dans ce fichier.');return;}
      // Ask target user
      var unames=USERS.map(function(u){return u.username;});
      var target=prompt('Importer ces '+conns.length+' connexions vers quel utilisateur ?\\n\\nUtilisateurs disponibles:\\n'+unames.join(', '));
      if(!target||!target.trim())return;
      var res=await api('/admin/connections/import',{target:target.trim(),connections:conns});
      if(res.ok){alert('✓ '+res.added+' connexion(s) ajoutée(s) (total: '+res.total+')');await loadAllConns();}
...
// ── HISTORIQUE ──
function buildHistoryFromConns(){
  // Collect history from all loaded user data (need separate API calls)
  // Use already-fetched data if available, otherwise load on demand
  if(!_allConnsLoaded)return;
  var histUser=document.getElementById('histUser');
  var users=[...new Set(USERS.map(function(u){return u.username;}))].sort();
  histUser.innerHTML='<option value="">Tous les utilisateurs</option>'+users.map(function(u){return'<option value="'+esc(u)+'">'+esc(u)+'</option>';}).join('');
  loadAllHistory();
}
var _allHist=[];
  ...
  var results=await Promise.all(USERS.map(async function(u){
    try{var r=await api('/admin/get',{target:u.username});
      if(r.ok){
        ...
        hist.forEach(function(h){h._user=u.username;});
        return hist;
      }
    }catch(e){}return[];
  }));
  results.forEach(function(h){_allHist=_allHist.concat(h);});
  _allHist.sort(function(a,b){return(b.time||b.ts||0)-(a.time||a.ts||0);});
  var apps=[...new Set(_allHist.map(function(h){return h.app||'?';}))].filter(function(a){return a!=='?';}).sort();
  var appSel=document.getElementById('histApp');
  appSel.innerHTML='<option value="">Toutes les apps</option>'+apps.map(function(a){return'<option value="'+esc(a)+'">'+esc(a)+'</option>';}).join('');
  renderHistory();
}
function renderHistory(){
  var q=(document.getElementById('histSearch').value||'').toLowerCase();
  var fu=document.getElementById('histUser').value;
  var fa=document.getElementById('histApp').value;
  var filtered=_allHist.filter(function(h){
    if(fu&&h._user!==fu)return false;
    if(fa&&(h.app||'')!==fa)return false;
    var text=(h.label||h.query||h.title||'').toLowerCase()+(h.url||'').toLowerCase();
    if(q&&!text.includes(q)&&!(h._user||'').toLowerCase().includes(q))return false;
    return true;
  });
  var list=document.getElementById('histList');
  if(!filtered.length){list.innerHTML='<div style="text-align:center;color:var(--muted);padding:30px;font-size:12px">Aucun historique trouvé</div>';return;}
  list.innerHTML='<div style="font-size:11px;color:var(--muted);padding:8px 12px;border-bottom:1px solid var(--border)">'+filtered.length+' entrée(s) — 200 max affichées</div>'+
  filtered.slice(0,500).map(function(h){
    var query=h.label||h.query||h.title||'(vide)';
    var url=h.url||'';
    var app=h.app||'';
    return '<div class="hist-item">'+
      '<div>'+
        '<div style="display:flex;align-items:center;gap:8px">'+
          '<span style="font-size:10px;color:var(--purple);background:rgba(160,107,255,.1);border-radius:10px;padding:2px 7px">'+esc(h._user||'?')+'</span>'+
          (app?'<span style="font-size:9px;color:var(--cyan);background:rgba(0,229,255,.1);border-radius:10px;padding:2px 7px">'+esc(app)+'</span>':'')+
        '</div>'+
        '<div style="font-weight:600;margin-top:4px">'+esc(query)+'</div>'+
        (url?'<div style="font-size:10px;color:var(--muted);font-family:monospace;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:500px">'+esc(url)+'</div>':'')+
      '</div>'+
      '<div style="text-align:right;font-size:10px;color:var(--muted);white-space:nowrap">'+fmtDate(h.time||h.ts)+'</div>'+
    '</div>';
  }).join('');
}
...
async function loadStats(){
  ...
  document.getElementById('statsContent').innerHTML='<div style="text-align:center;padding:40px;color:var(--muted)">⏳ Chargement des statistiques…</div>';
  var r=await apiGet('/admin/stats');
  if(!r||!r.ok){document.getElementById('statsContent').innerHTML='<div style="color:var(--red);padding:20px">Erreur: '+(r&&r.error||'')+'</div>';return;}
  _statsLoaded=true;
  // Render
  var html='<div class="grid3" style="margin-bottom:16px">'+
    '<div class="stat"><div class="stat-n" style="color:var(--cyan)">'+r.total_users+'</div><div class="stat-l">Utilisateurs</div></div>'+
    '<div class="stat"><div class="stat-n" style="color:var(--gold)">'+r.total_rewards.toLocaleString()+'</div><div class="stat-l">Rewards total</div></div>'+
    '<div class="stat"><div class="stat-n" style="color:var(--cyan)">'+r.total_nxc+' NXC</div><div class="stat-l">NXC en circulation</div></div>'+
  '</div>';
  // Roles
  html+='<div class="card"><div class="card-title">👥 Répartition des rôles</div><div class="grid2">'+
    '<div class="stat"><div class="stat-n" style="color:var(--pink)">'+r.roles.admin+'</div><div class="stat-l">👑 Admins</div></div>'+
    '<div class="stat"><div class="stat-n" style="color:var(--green)">'+r.roles.moderator+'</div><div class="stat-l">🛡️ Modérateurs</div></div>'+
    '<div class="stat"><div class="stat-n" style="color:var(--gold)">'+r.roles.vip+'</div><div class="stat-l">⭐ VIP</div></div>'+
    '<div class="stat"><div class="stat-n" style="color:var(--purple)">'+r.roles.user+'</div><div class="stat-l">👤 Utilisateurs</div></div>'+
  '</div></div>';
  // Charts
  html+='<div class="grid2">'+
    '<div class="card"><div class="card-title">📈 Connexions par jour</div><div class="chart-wrap"><canvas id="chartConns"></canvas></div></div>'+
    '<div class="card"><div class="card-title">🌍 Connexions par pays</div><div class="chart-wrap"><canvas id="chartCountries"></canvas></div></div>'+
  '</div>';
  html+='<div class="grid2">'+
    '<div class="card"><div class="card-title">📱 Apps Nexus utilisées</div><div class="chart-wrap"><canvas id="chartApps"></canvas></div></div>'+
    '<div class="card"><div class="card-title">🏆 Top 10 rewards</div><div id="topRewards"></div></div>'+
  '</div>';
  document.getElementById('statsContent').innerHTML=html;
  // Draw charts
  setTimeout(function(){
    var colors=['#00e5ff','#a06bff','#00ff9d','#ffb020','#ff4757','#ff5db1','#4ecdc4','#ffd93d','#6bcb77','#ff6b6b'];
    if(r.conns_by_day&&r.conns_by_day.length){
      new Chart(document.getElementById('chartConns'),{type:'line',data:{labels:r.conns_by_day.map(function(x){return x[0];}),datasets:[{label:'Connexions',data:r.conns_by_day.map(function(x){return x[1];}),borderColor:'#00e5ff',backgroundColor:'rgba(0,229,255,.08)',tension:.4,fill:true,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#3d4f7a',maxTicksLimit:8},grid:{color:'rgba(255,255,255,.04)'}},y:{ticks:{color:'#3d4f7a'},grid:{color:'rgba(255,255,255,.04)'}}}}});
    }
    if(r.countries&&r.countries.length){
      new Chart(document.getElementById('chartCountries'),{type:'doughnut',data:{labels:r.countries.slice(0,8).map(function(x){return x[0];}),datasets:[{data:r.countries.slice(0,8).map(function(x){return x[1];}),backgroundColor:colors,borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#dce8ff',font:{size:11}}}}}});
    }
    if(r.apps&&r.apps.length){
      new Chart(document.getElementById('chartApps'),{type:'bar',data:{labels:r.apps.map(function(x){return x[0];}),datasets:[{data:r.apps.map(function(x){return x[1];}),backgroundColor:colors,borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#3d4f7a'},grid:{color:'rgba(255,255,255,.04)'}},y:{ticks:{color:'#dce8ff'}}}}});
    }
    // Top rewards
    if(r.top_rewards){
      var tr=document.getElementById('topRewards');
      if(tr)tr.innerHTML=r.top_rewards.map(function(u,i){
        return'<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">'+
          '<span style="color:var(--muted);font-size:12px;width:20px">#'+(i+1)+'</span>'+
          '<span style="flex:1;font-size:12px">'+esc(u.user)+'</span>'+
          roleBadge(u.role)+
          '<span style="font-weight:700;color:var(--gold);font-size:12px">'+u.rewards.toLocaleString()+' R</span>'+
        '</div>';
      }).join('');
    }
  },100);
...
function initMessages(){
  ...
  sel.innerHTML='<option value="__all__">📢 Tous les utilisateurs</option>'+USERS.map(function(u){return'<option value="'+esc(u.username)+'">'+esc(u.username)+'</option>';}).join('');
  renderSentMessages();
}
function renderSentMessages(){
  var el=document.getElementById('sentMessages');
  if(!_sentMsgs.length){el.innerHTML='<div style="color:var(--muted);font-size:12px;text-align:center;padding:20px">Aucun message envoyé cette session</div>';return;}
  el.innerHTML=_sentMsgs.slice().reverse().map(function(m){
    return'<div class="msg-item">'+
      '<div class="msg-to">→ '+esc(m.target==='__all__'?'Tous les utilisateurs':m.target)+'</div>'+
      '<div class="msg-text">'+esc(m.text)+'</div>'+
      '<div class="msg-ts">'+fmtDate(m.ts)+'</div>'+
    '</div>';
  }).join('');
}
async function sendMessage(){
  ...
  if(!text){res.style.color='var(--red)';res.textContent='Message vide';return;}
  res.style.color='var(--muted)';res.textContent='Envoi…';
  var r=await api('/admin/message',{target:target,text:text});
  if(r.ok){
    res.style.color='var(--green)';res.textContent='✓ Envoyé à '+r.sent_to+' utilisateur(s)';
    _sentMsgs.push({target:target,text:text,ts:Date.now()});
    document.getElementById('msgText').value='';
    renderSentMessages();
  }else{res.style.color='var(--red)';res.textContent='Erreur: '+(r.error||'');}
}

// ── MONITOR ──
var _monChartInst=null;var _monData={labels:[],data:[]};
async function loadMonitor(){
  var r=await apiGet('/admin/logs');
  if(!r||!r.ok)return;
  var logs=r.logs||[];
  var uptime=r.uptime||0;
  document.getElementById('monUptime').textContent=fmtUptime(uptime);
  document.getElementById('monReqs').textContent=logs.length;
  var totalMs=logs.reduce(function(s,l){return s+(l.ms||0);},0);
  document.getElementById('monAvgMs').textContent=logs.length?Math.round(totalMs/logs.length)+'ms':'—';
  var errs=logs.filter(function(l){return(l.status||0)>=400;}).length;
  document.getElementById('monErrors').textContent=errs;
  // Log list
  document.getElementById('logList').innerHTML=logs.slice(0,50).map(function(l){
    var mc='m-'+l.method.toLowerCase();
    var sc='s-'+String(l.status||0)[0];
    return'<div class="log-row">'+
      '<span class="'+mc+'">'+esc(l.method)+'</span>'+
      '<span class="'+sc+'">'+esc(l.status||'?')+'</span>'+
      '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(l.path||'')+'</span>'+
      '<span style="color:var(--muted)">'+esc(l.ip||'')+'</span>'+
      '<span style="color:var(--gold)">'+esc(l.ms||0)+'</span>'+
    '</div>';
  }).join('');
  // Mini chart
  var now=new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
  var count=logs.length;
  _monData.labels.push(now);_monData.data.push(count);
  if(_monData.labels.length>20){_monData.labels.shift();_monData.data.shift();}
  var ctx=document.getElementById('monChart');
  if(!ctx)return;
  if(_monChartInst){_monChartInst.data.labels=_monData.labels;_monChartInst.data.datasets[0].data=_monData.data;_monChartInst.update('none');}
  else{_monChartInst=new Chart(ctx,{type:'line',data:{labels:_monData.labels,datasets:[{label:'Requêtes',data:_monData.data,borderColor:'#a06bff',backgroundColor:'rgba(160,107,255,.08)',tension:.4,fill:true,pointRadius:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#3d4f7a'}},y:{ticks:{color:'#3d4f7a'}}}}});}
}

// Auto-init messages when page shown
(function(){
  var tabs=document.querySelectorAll('.tab');
  tabs.forEach(function(t){
    t.addEventListener('click',function(){
      if(t.textContent.includes('Messages'))initMessages();
    });
  });
})();
</script>
