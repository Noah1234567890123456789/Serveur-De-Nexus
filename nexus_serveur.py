...
def _nxc_autotick():
    ...
    while True:
        try:
            ...
            # 1) Bruit de base — variation douce ~1 % (gaussienne, centrée sur 0)
            ...
            # 3) Rappel vers la référence (mean-reversion) — empêche la dérive
            #    éternelle : plus le prix s'éloigne, plus il est ramené.
            mr = 0.0
            if ref > 0:
                gap = (ref - p) / p                             # écart relatif
                mr = gap * 0.06                                 # 6 % de l'écart/tick

            # 4) Biais directionnel optionnel (panel Contrôle → tendance)
            drift_adj = NXC_BIAS.get("drift", 0) * 0.02 * NXC_BIAS.get("speed", 1.0)

            adj = noise + shock + mr + drift_adj
            p = p * (1 + adj)
