...
def _r2_bucket():
    return os.environ.get('R2_BUCKET_NAME', 'nexus-store')
...
def _cf_stream_customer():
    return os.environ.get('CF_STREAM_CUSTOMER', '')

def _store_items_load():
    raw = rdb.get('nexus_store_items')
    return json.loads(raw) if raw else {}

def _store_items_save(d):
    rdb.set('nexus_store_items', json.dumps(d))

def _store_unlocks_load():
    raw = rdb.get('nexus_store_unlocks')
    return json.loads(raw) if raw else {}

def _store_unlocks_save(d):
    rdb.set('nexus_store_unlocks', json.dumps(d))
...
def _r2_presigned_upload(key, content_type='application/octet-stream', expires=3600):
    c = _r2_client()
    if not c:
        return None
    try:
        return c.generate_presigned_url('put_object', Params={
            'Bucket': _r2_bucket(), 'Key': key, 'ContentType': content_type
        }, ExpiresIn=expires)
    except Exception:
        return None

def _r2_presigned_download(key, filename, expires=3600):
    c = _r2_client()
    if not c:
        return None
    try:
        return c.generate_presigned_url('get_object', Params={
...
def _cf_stream_token(video_id, expires_seconds=172800):
    acct = _cf_account_id()
    tok = _cf_api_token()
    ...
    try:
        if _REQUESTS_AVAILABLE:
            r = _requests.post(
                f'https://api.cloudflare.com/client/v4/accounts/{acct}/stream/{video_id}/token',
                headers={'Authorization': f'Bearer {tok}', 'Content-Type': 'application/json'},
                json={'exp': int(time.time()) + expires_seconds},
                timeout=10
            )
            if r.ok:
            ...
            body = _j.dumps({'exp': int(time.time()) + expires_seconds}).encode()
            req = _u.Request(
                f'https://api.cloudflare.com/client/v4/accounts/{acct}/stream/{video_id}/token',
            ...
            with _u.urlopen(req, timeout=10) as resp:
                return _j.loads(resp.read()).get('result', {}).get('token')
    except Exception:
        pass
    return None

def _cf_stream_direct_upload():
    acct = _cf_account_id()
    tok = _cf_api_token()
    if not acct or not tok:
        return None, None
    try:
        if _REQUESTS_AVAILABLE:
            r = _requests.post(
                f'https://api.cloudflare.com/client/v4/accounts/{acct}/stream/direct_upload',
                headers={'Authorization': f'Bearer {tok}', 'Content-Type': 'application/json'},
                json={'maxDurationSeconds': 21600, 'requireSignedURLs': True},
                timeout=10
            )
            if r.ok:
                res = r.json().get('result', {})
                return res.get('uploadURL'), res.get('uid')
        else:
            import json as _j
            import urllib.request as _u
            body = _j.dumps({'maxDurationSeconds': 21600, 'requireSignedURLs': True}).encode()
            req = _u.Request(
                f'https://api.cloudflare.com/client/v4/accounts/{acct}/stream/direct_upload',
                data=body, method='POST',
                headers={'Authorization': f'Bearer {tok}', 'Content-Type': 'application/json'}
            ...
            with _u.urlopen(req, timeout=10) as resp:
                res = _j.loads(resp.read()).get('result', {})
                return res.get('uploadURL'), res.get('uid')
    except Exception:
        pass
    return None, None

def _store_auth(data):
    ...
    with _lock:
        db = load_db()
    ud = db.get('users', {}).get(username)
    if not ud:
        return None, None
    if ud.get('password') != _pw_hash(password):
        return None, None
    return ud, username


# ═══════════════════════════════════════════════════════════════════════════════
# ◈ NEXUS STORE — routes utilisateur
# ═══════════════════════════════════════════════════════════════════════════════
...
@app.post('/store/api/items')
def store_api_items():
    items = _store_items_load()
    pub = []
    ...
    pub.sort(key=lambda x: x['created_at'], reverse=True)
    return jsonify(ok=True, items=pub)

@app.post('/store/api/my_items')
def store_api_my_items():
    data = request.json or {}
    ud, username = _store_auth(data)
    if not ud:
        return jsonify(ok=False, error='Auth incorrecte'), 401
    unlocks = _store_unlocks_load()
    items = _store_items_load()
    user_unlocks = unlocks.get(username, {})
    now = int(time.time())
    result = []
    for iid, udata in user_unlocks.items():
        item = items.get(iid)
        if not item:
            continue
        expired = False
        if item.get('type') == 'video_rental':
            exp = udata.get('expires_at') or 0
            if exp and now > exp:
                expired = True
        result.append({
            'id': iid, 'name': item.get('name', ''),
            'type': item.get('type', 'file'),
            'description': item.get('description', ''),
            'unlocked_at': udata.get('unlocked_at'),
            'expires_at': udata.get('expires_at'),
            'expired': expired,
        })
    return jsonify(ok=True, items=result, username=username, role=ud.get('role', 'user'))

@app.post('/store/api/unlock')
def store_api_unlock():
    ...
    if _code_hash(code) != item.get('code_hash', ''):
        return jsonify(ok=False, error='Code incorrect ❌'), 403
    unlocks = _store_unlocks_load()
    if username not in unlocks:
        unlocks[username] = {}
    now = int(time.time())
    if item_id not in unlocks[username]:
        expires_at = (now + 48 * 3600) if item.get('type') == 'video_rental' else None
        unlocks[username][item_id] = {'unlocked_at': now, 'expires_at': expires_at}
        _store_unlocks_save(unlocks)
    return jsonify(ok=True, item_type=item.get('type', 'file'))

@app.post('/store/api/download')
def store_api_download():
    ...
    if item_id not in unlocks.get(username, {}):
        return jsonify(ok=False, error='Article non débloqué'), 403
    items = _store_items_load()
    item = items.get(item_id)
    if not item:
        return jsonify(ok=False, error='Article introuvable'), 404
    if item.get('type') != 'file':
        return jsonify(ok=False, error='Pas un fichier téléchargeable'), 400
    file_key = item.get('file_key')
    file_name = item.get('file_name', 'download')
    if not file_key:
        return jsonify(ok=False, error='Fichier non disponible'), 404
    url = _r2_presigned_download(file_key, file_name)
    if not url:
        return jsonify(ok=False, error='Erreur stockage R2'), 500
    return jsonify(ok=True, url=url, filename=file_name)

@app.post('/store/api/stream')
def store_api_stream():
    ...
    unlocks = _store_unlocks_load()
    user_unlocks = unlocks.get(username, {})
    if item_id not in user_unlocks:
        return jsonify(ok=False, error='Article non débloqué'), 403
    items = _store_items_load()
    item = items.get(item_id)
    if not item:
        return jsonify(ok=False, error='Article introuvable'), 404
    if item.get('type') != 'video_rental':
        return jsonify(ok=False, error='Pas une vidéo'), 400
    udata = user_unlocks[item_id]
    expires_at = udata.get('expires_at') or 0
    now = int(time.time())
    if expires_at and now > expires_at:
        return jsonify(ok=False, error='Location expirée — 48h dépassées', expired=True), 403
    video_id = item.get('video_id')
    if not video_id:
        return jsonify(ok=False, error='Vidéo non disponible'), 404
    remaining = max(expires_at - now, 300) if expires_at else 172800
    token = _cf_stream_token(video_id, expires_seconds=remaining)
    if not token:
        return jsonify(ok=False, error='Erreur token vidéo (CF Stream non configuré?)'), 500
    customer = _cf_stream_customer()
    iframe_url = (
        f'https://customer-{customer}.cloudflarestream.com/{token}/iframe'
        if customer else
        f'https://iframe.cloudflarestream.com/{token}'
    )
    return jsonify(ok=True, iframe_url=iframe_url, expires_at=expires_at)

@app.post('/store/api/note')
def store_api_note():
    data = request.json or {}
    ud, username = _store_auth(data)
    if not ud:
    ...
    if item.get('type') != 'note':
        return jsonify(ok=False, error='Pas une note'), 400
    return jsonify(ok=True, name=item.get('name', ''), content=item.get('note_content', ''))


# ═══════════════════════════════════════════════════════════════════════════════
# ◈ NEXUS STORE — routes admin
# ═══════════════════════════════════════════════════════════════════════════════

def _store_admin_check(data):
    mk = data.get('master_key', '')
    return mk and secrets.compare_digest(mk, MASTER_KEY)

@app.post('/store/admin/list')
def store_admin_list():
    data = request.json or {}
    if not _store_admin_check(data):
        return jsonify(ok=False, error='Non autorisé'), 403
    return jsonify(ok=True, items=_store_items_load())

@app.post('/store/admin/create')
def store_admin_create():
    ...
    if not name or not code:
        return jsonify(ok=False, error='Nom et code requis'), 400
    if item_type not in ('file', 'note', 'video_rental'):
        return jsonify(ok=False, error='Type invalide'), 400
    items = _store_items_load()
    iid = str(_uuid.uuid4())[:12]
    items[iid] = {
        'id': iid, 'name': name, 'type': item_type,
        'code_hash': _code_hash(code),
        'description': description,
        'note_content': note_content if item_type == 'note' else '',
        'file_key': None, 'file_name': None, 'video_id': None,
        'created_at': int(time.time()), 'updated_at': int(time.time()),
    }
    _store_items_save(items)
    return jsonify(ok=True, id=iid)

@app.post('/store/admin/edit')
def store_admin_edit():
    ...
    if item_id not in items:
        return jsonify(ok=False, error='Article introuvable'), 404
    item = items[item_id]
    for field in ('name', 'description', 'note_content', 'file_key', 'file_name', 'video_id'):
        if field in data:
            item[field] = data[field]
    if data.get('code'):
        item['code_hash'] = _code_hash(data['code'])
    item['updated_at'] = int(time.time())
    _store_items_save(items)
    return jsonify(ok=True)

@app.post('/store/admin/delete')
def store_admin_delete():
    data = request.json or {}
    if not _store_admin_check(data):
        return jsonify(ok=False, error='Non autorisé'), 403
    item_id = (data.get('item_id') or '').strip()
    ...
    if item.get('file_key'):
        try:
            c = _r2_client()
            if c:
                c.delete_object(Bucket=_r2_bucket(), Key=item['file_key'])
        except Exception:
            pass
    del items[item_id]
    _store_items_save(items)
...
@app.post('/store/admin/upload_url')
def store_admin_upload_url():
    data = request.json or {}
    if not _store_admin_check(data):
        return jsonify(ok=False, error='Non autorisé'), 403
    filename = data.get('filename', 'file')
    content_type = data.get('content_type', 'application/octet-stream')
    key = f"store/{int(time.time())}_{_uuid.uuid4().hex[:8]}_{filename}"
    url = _r2_presigned_upload(key, content_type)
    if not url:
        return jsonify(ok=False, error='R2 non configuré — vérifiez les variables R2_*'), 500
    return jsonify(ok=True, upload_url=url, key=key, filename=filename)

@app.post('/store/admin/stream_upload')
def store_admin_stream_upload():
    data = request.json or {}
    if not _store_admin_check(data):
        return jsonify(ok=False, error='Non autorisé'), 403
    upload_url, uid = _cf_stream_direct_upload()
    if not upload_url:
        return jsonify(ok=False, error='CF Stream non configuré — vérifiez CF_API_TOKEN'), 500
    return jsonify(ok=True, upload_url=upload_url, video_id=uid)

@app.post('/store/admin/stats')
def store_admin_stats():
    data = request.json or {}
    if not _store_admin_check(data):
        return jsonify(ok=False, error='Non autorisé'), 403
    unlocks = _store_unlocks_load()
    ...
    now = int(time.time())
    result = {}
    for username, user_unlocks in unlocks.items():
        result[username] = []
        for iid, udata in user_unlocks.items():
            item = items.get(iid, {})
            expired = False
            ...
            result[username].append({
                'item_id': iid, 'item_name': item.get('name', '[supprimé]'),
                'item_type': item.get('type', '?'),
                'unlocked_at': udata.get('unlocked_at'),
                'expires_at': udata.get('expires_at'),
                'expired': expired,
            })
    total_unlocks = sum(len(v) for v in result.values())
    return jsonify(ok=True, unlocks=result, total_items=len(items), total_unlocks=total_unlocks)
