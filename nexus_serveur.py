...
def _nxc_autotick():
    ...
    while True:
        try:
            ...
            # Plancher anti-chute : ne pas descendre sous 70% du prix de référence admin
            ref_price = NXC_MARKET.get("admin_price", 0)
            if ref_price > 0 and p < ref_price * 0.70:
                p = ref_price * 0.70
                p = round(p, 2)
            p = round(p * 100) / 100
            NXC_MARKET["price"] = p
...
def nxc_tick():
    ...
    NXC_MARKET["admin_price"] = price  # référence pour le plancher -30%
    # La cible suit le nouveau prix — aucune attraction vers une valeur passée
    NXC_MEAN_PRICE["target"] = price
    NXC_MEAN_PRICE["enabled"] = False  # s'assurer que la mean-reversion reste off
