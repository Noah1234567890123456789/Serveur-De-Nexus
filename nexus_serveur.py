    ...
    while True:
        try:
            ...
            if _grow_on and not _g.get("combine", True):
                ...
                # 1) Bruit de base — variation douce ~1 % (gaussienne centrée)
                sigma = (0.007 + _rnd.random() * 0.008) * volt      # 0.7 % – 1.5 %
                noise = _rnd.gauss(0.0, 1.0) * sigma
                # 2) Choc rare — ~3 % de chance d'un gros mouvement jusqu'à ±30 %
                shock = 0.0
                if _rnd.random() < 0.03:
                    mag = (0.05 + _rnd.random() * 0.25) * volt      # 5 % – 30 %
                    shock = mag if _rnd.random() < 0.5 else -mag
            ...
            # 5) Bornes de sécurité relatives à la référence
            if ref > 0:
                p = max(ref * 0.70, min(ref * 2.5, p))          # plancher -30 %
            p = max(_NXC_PRICE_MIN, min(_NXC_PRICE_MAX, p))     # bornes absolues
            p = round(p * 100) / 100
...
def nxc_price():
    ...
    with _lock:
        ...
        if now_ms - last_ts > 10000:
            ...
            p = float(NXC_MARKET["price"])
            ref = float(NXC_MARKET.get("admin_price") or p)
            sigma = (0.007 + _rnd.random() * 0.008) * volt
            noise = _rnd.gauss(0.0, 1.0) * sigma
            shock = 0.0
            if _rnd.random() < 0.03:
                mag = (0.05 + _rnd.random() * 0.25) * volt
                shock = mag if _rnd.random() < 0.5 else -mag
