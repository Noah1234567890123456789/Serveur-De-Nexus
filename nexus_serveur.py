...
# PANNEAU NXC COIN
# ══════════════════════════════════════════════════════════
NXC_PANEL_HTML = '<!DOCTYPE html>\n<html lang="fr">\n<head>\n<meta charset="utf-8">\n<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover">\n<title>◈ Nexus Server</title>\n<style>\n*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;touch-action:manipulation}\n:root{\n  --bg:#02040a;--bg2:#080d1a;--bg3:#0d1428;\n  --cyan:#00e5ff;--green:#00ff9d;--red:#ff3d5e;--gold:#ffb020;--purple:#a06bff;--blue:#4ea8de;\n  --muted:#5c6b8c;--text:#d4e8ff;--border:rgba(0,229,255,.12);\n}\nhtml{background:var(--bg);-webkit-text-size-adjust:100%}\nbody{background:var(--bg);color:var(--text);font-family:\'Segoe UI\',system-ui,sans-serif;min-height:100vh;min-height:100dvh;overflow-x:hidden}\n\n/* ── HUD ── */\n.hud{position:fixed;top:0;left:0;right:0;height:56px;background:rgba(2,4,10,.95);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 14px;gap:10px;z-index:200;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}\n.hud-logo{font-family:monospace;font-size:16px;font-weight:900;color:var(--cyan);letter-spacing:2px;flex-shrink:0}\n.hud-logo span{color:#fff;opacity:.35}\n.hud-sep{width:1px;height:24px;background:var(--border);flex-shrink:0}\n.hud-price{font-family:monospace;font-size:13px;font-weight:800;color:var(--cyan);flex-shrink:0}\n.hud-chg{font-size:10px;font-weight:700;padding:2px 7px;border-radius:20px;flex-shrink:0}\n.hud-chg.up{background:rgba(0,255,157,.12);color:var(--green);border:1px solid rgba(0,255,157,.2)}\n.hud-chg.dn{background:rgba(255,61,94,.12);color:var(--red);border:1px solid rgba(255,61,94,.2)}\n.hud-trend{font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;background:rgba(255,176,32,.1);border:1px solid rgba(255,176,32,.2);color:var(--gold);display:none;flex-shrink:0}\n.hud-right{margin-left:auto;display:flex;align-items:center;gap:8px}\n.hud-dot{width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 8px var(--green);animation:dp 2s infinite;flex-shrink:0}\n@keyframes dp{0%,100%{opacity:1}50%{opacity:.3}}\n.hud-time{font-family:monospace;font-size:11px;color:var(--muted)}\n.hud-link{padding:5px 10px;border:1px solid var(--border);border-radius:8px;font-size:10px;font-weight:700;color:var(--muted);text-decoration:none;background:rgba(255,255,255,.04);white-space:nowrap}\n.hud-link:hover{border-color:var(--cyan);color:var(--cyan)}\n.hud-link.primary{border-color:rgba(0,229,255,.3);color:var(--cyan);background:rgba(0,229,255,.06)}\n\n/* ── LOGIN ── */\n#ls{position:fixed;inset:0;background:var(--bg);z-index:999;display:flex;align-items:center;justify-content:center;padding:20px}\n.lb{background:var(--bg2);border:1px solid var(--border);border-radius:22px;padding:32px 24px;width:100%;max-width:360px;text-align:center}\n.lb-logo{font-family:monospace;font-size:28px;font-weight:900;color:var(--cyan);letter-spacing:3px;margin-bottom:4px}\n.lb-sub{font-size:12px;color:var(--muted);margin-bottom:28px;letter-spacing:.5px}\n.fi{width:100%;padding:13px 14px;background:var(--bg3);border:1px solid var(--border);border-radius:11px;color:var(--text);font-size:16px;margin-bottom:10px;outline:none;transition:.2s}\n.fi:focus{border-color:var(--cyan);box-shadow:0 0 0 3px rgba(0,229,255,.1)}\n.btn{width:100%;padding:13px;border-radius:11px;font-size:14px;font-weight:800;cursor:pointer;border:none;letter-spacing:.5px;transition:.2s}\n.btn.primary{background:linear-gradient(135deg,var(--cyan),#0097b2);color:#000}\n.btn.primary:active{opacity:.85}\n#lm{font-size:12px;color:var(--red);margin-top:8px;min-height:16px}\n\n/* ── TABS ── */\n.tabs{position:fixed;bottom:0;left:0;right:0;background:rgba(2,4,10,.97);border-top:1px solid var(--border);display:flex;z-index:100;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);padding-bottom:env(safe-area-inset-bottom)}\n.tab{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:8px 4px 10px;gap:3px;cursor:pointer;border:none;background:none;color:var(--muted);font-size:9px;font-weight:700;letter-spacing:.3px;transition:.15s;position:relative}\n.tab-ico{font-size:20px;line-height:1}\n.tab.on{color:var(--cyan)}\n.tab.on::before{content:\'\';position:absolute;top:0;left:20%;right:20%;height:2px;background:var(--cyan);border-radius:0 0 3px 3px}\n.tab .notif-dot{position:absolute;top:6px;right:calc(50% - 14px);width:7px;height:7px;background:var(--red);border-radius:50%;display:none;border:2px solid var(--bg)}\n\n/* ── CONTENT ── */\n.content{padding-top:56px;padding-bottom:calc(64px + env(safe-area-inset-bottom))}\n.view{display:none;padding:14px;max-width:900px;margin:0 auto}\n.view.on{display:block}\n\n/* ── CARDS ── */\n.card{background:var(--bg2);border:1px solid var(--border);border-radius:16px;padding:16px;margin-bottom:12px;position:relative}\n.card.cyan{border-color:rgba(0,229,255,.25)}\n.card.green{border-color:rgba(0,255,157,.25)}\n.card.red{border-color:rgba(255,61,94,.25)}\n.card.gold{border-color:rgba(255,176,32,.25)}\n.card.purple{border-color:rgba(160,107,255,.25)}\n.ct{font-size:9px;letter-spacing:2px;color:var(--muted);margin-bottom:12px;font-weight:700;text-transform:uppercase}\n.g4{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px}\n.g3{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px}\n.g2{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px}\n.st{background:var(--bg3);border:1px solid rgba(0,229,255,.07);border-radius:12px;padding:12px 8px;text-align:center}\n.sv{font-family:monospace;font-size:18px;font-weight:800;color:var(--cyan);margin-bottom:2px;line-height:1.1}\n.sl{font-size:8px;color:var(--muted);letter-spacing:.8px;text-transform:uppercase}\n.sv.gold{color:var(--gold)}.sv.green{color:var(--green)}.sv.red{color:var(--red)}.sv.purple{color:var(--purple)}\n\n/* ── FORM ELEMENTS ── */\ninput,select,textarea{width:100%;padding:12px 13px;background:var(--bg3);border:1px solid var(--border);border-radius:11px;color:var(--text);font-size:14px;margin-bottom:8px;outline:none;transition:.2s;font-family:inherit}\ninput:focus,select:focus{border-color:var(--cyan);box-shadow:0 0 0 3px rgba(0,229,255,.08)}\n.row{display:flex;gap:8px;align-items:center}\n.grow{flex:1;min-width:0;margin-bottom:0}\n.btn-sm{padding:10px 14px;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;border:1px solid var(--border);background:var(--bg3);color:var(--text);white-space:nowrap;flex-shrink:0}\n.btn-sm.cyan{background:rgba(0,229,255,.1);border-color:rgba(0,229,255,.3);color:var(--cyan)}\n.btn-sm.green{background:rgba(0,255,157,.1);border-color:rgba(0,255,157,.3);color:var(--green)}\n.btn-sm.red{background:rgba(255,61,94,.1);border-color:rgba(255,61,94,.3);color:var(--red)}\n.btn-sm.gold{background:rgba(255,176,32,.1);border-color:rgba(255,176,32,.3);color:var(--gold)}\n.btn-sm.purple{background:rgba(160,107,255,.1);border-color:rgba(160,107,255,.3);color:var(--purple)}\n.btn-full{width:100%;padding:12px;border-radius:11px;font-size:13px;font-weight:700;cursor:pointer;border:none;margin-bottom:8px}\n.btn-full.primary{background:linear-gradient(135deg,var(--cyan),#0097b2);color:#000}\n.btn-full.danger{background:rgba(255,61,94,.12);border:1px solid rgba(255,61,94,.3);color:var(--red)}\n.btn-full.warn{background:rgba(255,176,32,.1);border:1px solid rgba(255,176,32,.3);color:var(--gold)}\n.btn-full.muted{background:rgba(255,255,255,.04);border:1px solid var(--border);color:var(--muted)}\n\n/* ── ALERTS ── */\n.ab{padding:10px 13px;border-radius:10px;font-size:12px;margin-bottom:6px;line-height:1.5}\n.ao{background:rgba(0,255,157,.07);border:1px solid rgba(0,255,157,.18);color:var(--green)}\n.aw{background:rgba(255,176,32,.07);border:1px solid rgba(255,176,32,.18);color:var(--gold)}\n.ae{background:rgba(255,61,94,.07);border:1px solid rgba(255,61,94,.18);color:var(--red)}\n.ai{background:rgba(0,229,255,.07);border:1px solid rgba(0,229,255,.18);color:var(--cyan)}\n.msg{font-size:11px;padding:7px 11px;border-radius:8px;margin-top:6px;font-weight:600}\n.msg.ok{background:rgba(0,255,157,.1);color:var(--green)}\n.msg.err{background:rgba(255,61,94,.1);color:var(--red)}\n\n/* ── CHART ── */\n.chart-wrap{height:200px;position:relative;margin:-4px 0 10px}\n.chart-btns{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}\n\n/* ── TABLE ── */\ntable{width:100%;border-collapse:collapse;font-size:12px}\nth,td{padding:10px 8px;text-align:left;border-bottom:1px solid rgba(0,229,255,.05)}\nth{color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:.5px;font-weight:700}\ntr:hover td{background:rgba(0,229,255,.02)}\n\n/* ── FLUX ── */\n.fl{padding:10px 12px;border-bottom:1px solid rgba(0,229,255,.05);display:flex;justify-content:space-between;align-items:center;gap:8px;font-size:12px}\n.fl:last-child{border-bottom:none}\n.fl-in{color:var(--green);font-weight:700;flex-shrink:0}\n.fl-out{color:var(--red);font-weight:700;flex-shrink:0}\n.fl-user{color:var(--muted);font-size:11px;flex:1;text-align:center}\n.fl-time{color:var(--muted);font-size:10px;text-align:right;flex-shrink:0}\n\n/* ── FAIL ITEMS ── */\n.fail{padding:12px;border-bottom:1px solid rgba(255,61,94,.1);display:flex;flex-direction:column;gap:6px;font-size:12px}\n.fail-top{display:flex;justify-content:space-between;align-items:center}\n\n/* ── TOGGLE ── */\n.tg{width:44px;height:24px;background:rgba(255,255,255,.08);border:1px solid var(--border);border-radius:12px;cursor:pointer;position:relative;flex-shrink:0;transition:.2s}\n.tg.on{background:rgba(0,229,255,.2);border-color:var(--cyan)}\n.tg-k{position:absolute;top:3px;left:3px;width:16px;height:16px;background:#fff;border-radius:50%;transition:.2s}\n.tg.on .tg-k{left:23px;background:var(--cyan)}\n\n/* ── NEXUS APP ── */\n#view-nexus .iframe-bar{background:var(--bg2);border:1px solid var(--border);border-radius:14px 14px 0 0;padding:10px 14px;display:flex;align-items:center;gap:10px}\n#view-nexus .iframe-url{flex:1;font-size:10px;color:var(--muted);font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}\n#nexus-frame{width:100%;height:calc(100dvh - 56px - 64px - 48px);border:none;border:1px solid var(--border);border-top:none;border-radius:0 0 14px 14px;background:var(--bg)}\n\n/* ── SEARCH ── */\n.search-wrap{position:relative;margin-bottom:10px}\n.search-wrap input{padding-left:36px;margin:0}\n.search-ico{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:14px;pointer-events:none}\n\n/* ── DESKTOP overrides ── */\n@media(min-width:768px){\n  .tabs{top:56px;bottom:auto;border-top:none;border-bottom:1px solid var(--border)}\n  .tab{flex-direction:row;gap:7px;padding:14px 20px;font-size:12px;justify-content:center}\n  .tab-ico{font-size:15px}\n  .tab.on::before{top:auto;bottom:0;left:10%;right:10%;border-radius:3px 3px 0 0}\n  .content{padding-top:112px;padding-bottom:20px}\n  .view{padding:20px}\n  .g4{grid-template-columns:repeat(4,1fr)}\n  .g3{grid-template-columns:repeat(3,1fr)}\n  .sv{font-size:22px}\n  .chart-wrap{height:240px}\n  #nexus-frame{height:calc(100vh - 112px - 64px)}\n  .hud-time{display:block}\n}\n@media(max-width:480px){\n  .g4{grid-template-columns:repeat(2,1fr)}\n  .g3{grid-template-columns:repeat(3,1fr)}\n  .sv{font-size:15px}\n}\n</style>\n</head>\n<body>\n\n<!-- LOGIN -->\n<div id="ls">\n  <div class="lb">\n    <div class="lb-logo">◈ NEXUS</div>\n    <div class="lb-sub">PANNEAU SERVEUR</div>\n    <input id="mk" type="password" placeholder="Clé maître" class="fi" onkeydown="if(event.key===\'Enter\')conn()">\n    <button class="btn primary" onclick="conn()">Connexion</button>\n    <div id="lm"></div>\n  </div>\n</div>\n\n<!-- HUD -->\n<div class="hud">\n  <div class="hud-logo">◈ <span>N</span>XC</div>\n  <div class="hud-sep"></div>\n  <div class="hud-price" id="hp">—</div>\n  <div class="hud-chg" id="hc"></div>\n  <div class="hud-trend" id="ht"></div>\n  <div class="hud-right">\n    <div class="hud-dot" id="hd" style="background:var(--muted);box-shadow:none"></div>\n    <div class="hud-time" id="htm" style="display:none">—</div>\n    <a href="/panel" class="hud-link">Admin</a>\n    <a href="https://lively-art-86d9.noah-guetta.workers.dev/nexus_coin_90.html" target="_blank" class="hud-link primary">◈ App ↗</a>\n  </div>\n</div>\n\n<!-- TABS -->\n<div class="tabs">\n  <button class="tab on" id="t-marche" onclick="go(\'marche\',this)"><span class="tab-ico">📈</span>Marché</button>\n  <button class="tab" id="t-banque" onclick="go(\'banque\',this)"><span class="tab-ico">🏦</span>Banque<span class="notif-dot" id="nd-banque"></span></button>\n  <button class="tab" id="t-users" onclick="go(\'users\',this)"><span class="tab-ico">👥</span>Comptes</button>\n  <button class="tab" id="t-solv" onclick="go(\'solv\',this)"><span class="tab-ico">🛡️</span>Solva.</button>\n  <button class="tab" id="t-nexus" onclick="go(\'nexus\',this)"><span class="tab-ico">🌐</span>App</button>\n</div>\n\n<!-- CONTENT -->\n<div class="content">\n\n  <!-- MARCHÉ -->\n  <div class="view on" id="view-marche">\n    <div class="g4">\n      <div class="st"><div class="sv" id="s-p">—</div><div class="sl">Prix R/NXC</div></div>\n      <div class="st"><div class="sv gold" id="s-v">—</div><div class="sl">Vol. 24h</div></div>\n      <div class="st"><div class="sv green" id="s-t">—</div><div class="sl">Trades 24h</div></div>\n      <div class="st"><div class="sv purple" id="s-h">—</div><div class="sl">Hist. pts</div></div>\n    </div>\n\n    <div class="card cyan">\n      <div class="ct">◈ HISTORIQUE DU COURS</div>\n      <div class="chart-wrap"><canvas id="ch"></canvas></div>\n      <div class="chart-btns">\n        <button class="btn-sm gold" onclick="chObj&&chObj.zoom(1.5)">+ Zoom</button>\n        <button class="btn-sm gold" onclick="chObj&&chObj.zoom(0.7)">− Dézoom</button>\n        <button class="btn-sm" onclick="chObj&&chObj.resetZoom()">Reset</button>\n      </div>\n    </div>\n\n    <div class="g2">\n      <div class="card">\n        <div class="ct">◈ MODIFIER LE COURS</div>\n        <input id="np" type="number" min="50" max="100000" placeholder="Nouveau prix (50–100 000)">\n        <div class="row" style="gap:6px">\n          <button class="btn-full primary" style="margin:0" onclick="setP()">✓ Appliquer</button>\n          <button class="btn-sm warn" onclick="resetH()" style="padding:10px 12px">🔄</button>\n        </div>\n        <div id="pm" style="min-height:16px;margin-top:6px;font-size:11px"></div>\n      </div>\n      <div class="card">\n        <div class="ct">◈ TENDANCE</div>\n        <select id="ts" style="margin-bottom:10px">\n          <option value="0.002">Très lent 0.2%</option>\n          <option value="0.005" selected>Lent 0.5%</option>\n          <option value="0.01">Moyen 1%</option>\n          <option value="0.02">Rapide 2%</option>\n          <option value="0.05">Très rapide 5%</option>\n        </select>\n        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">\n          <button class="btn-sm green" onclick="setT(\'up\')">📈 Hausse</button>\n          <button class="btn-sm red" onclick="setT(\'down\')">📉 Baisse</button>\n          <button class="btn-sm purple" onclick="setT(\'random\')">🎲 Aléa.</button>\n          <button class="btn-sm" onclick="setT(\'stop\')">⏸ Stop</button>\n        </div>\n        <div id="tst" style="font-size:11px;color:var(--muted);margin-top:8px;font-weight:600">⏸ Arrêté</div>\n      </div>\n    </div>\n\n    <div class="card">\n      <div class="ct">◈ ALERTES MARCHÉ</div>\n      <div id="al"></div>\n    </div>\n  </div>\n\n  <!-- BANQUE -->\n  <div class="view" id="view-banque">\n    <div class="g4">\n      <div class="st"><div class="sv" style="color:#00b4d8;font-size:15px" id="bk-r">—</div><div class="sl">Réserves R</div></div>\n      <div class="st"><div class="sv gold" style="font-size:15px" id="bk-i">—</div><div class="sl">Total entré</div></div>\n      <div class="st"><div class="sv red" style="font-size:15px" id="bk-o">—</div><div class="sl">Total sorti</div></div>\n      <div class="st"><div class="sv green" style="font-size:15px" id="bk-rt">—</div><div class="sl">Ratio</div></div>\n    </div>\n\n    <div class="card cyan">\n      <div class="ct">◈ OPÉRATIONS</div>\n      <div class="row" style="margin-bottom:10px">\n        <input id="bk-amt" type="number" min="1" placeholder="Montant (R)" class="grow">\n        <button class="btn-sm green" onclick="bankInject()">+ Injecter</button>\n        <button class="btn-sm red" onclick="bankRetire()">− Retirer</button>\n      </div>\n      <div style="display:flex;gap:6px;flex-wrap:wrap">\n        <button class="btn-sm warn" onclick="bankResetHist()">🗑️ Reset historique</button>\n        <button class="btn-sm red" onclick="bankResetAll()">💥 Reset complet</button>\n        <button class="btn-sm cyan" onclick="loadBank()">🔄 Actualiser</button>\n      </div>\n      <div id="bk-msg" style="min-height:14px;margin-top:8px;font-size:11px;font-weight:600"></div>\n    </div>\n\n    <div class="card">\n      <div class="ct">◈ DERNIERS FLUX</div>\n      <div id="bk-flux" style="max-height:220px;overflow-y:auto"></div>\n    </div>\n\n    <div class="card red">\n      <div class="ct" style="color:var(--red)">⚠️ TENTATIVES ÉCHOUÉES</div>\n      <div id="bk-fails" style="max-height:220px;overflow-y:auto"></div>\n    </div>\n  </div>\n\n  <!-- COMPTES -->\n  <div class="view" id="view-users">\n    <div class="card">\n      <div class="ct">◈ UTILISATEURS</div>\n      <div class="search-wrap">\n        <span class="search-ico">🔍</span>\n        <input id="us-q" placeholder="Rechercher..." oninput="filterU()" style="margin:0">\n      </div>\n      <div id="ut-wrap" style="overflow-x:auto">\n        <table>\n          <thead><tr><th>Compte</th><th>Rôle</th><th>Rewards</th><th>NXC</th><th>Valeur</th></tr></thead>\n          <tbody id="ut"></tbody>\n        </table>\n      </div>\n      <div id="us-msg" style="font-size:11px;color:var(--muted);margin-top:8px;text-align:center"></div>\n    </div>\n  </div>\n\n  <!-- SOLVABILITÉ -->\n  <div class="view" id="view-solv">\n    <div class="card">\n      <div class="ct">◈ CONTRÔLE DE SOLVABILITÉ</div>\n      <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;padding:14px;background:var(--bg3);border-radius:12px;cursor:pointer" onclick="toggleSolv()">\n        <div class="tg" id="stg"><div class="tg-k"></div></div>\n        <div>\n          <div id="sl" style="font-size:14px;font-weight:700;color:var(--muted)">Désactivée</div>\n          <div style="font-size:11px;color:var(--muted);margin-top:2px">Cliquer pour activer/désactiver</div>\n        </div>\n      </div>\n      <div class="row" style="margin-bottom:8px">\n        <span style="font-size:12px;color:var(--muted);white-space:nowrap;flex-shrink:0">Geste commercial :</span>\n        <input id="sg" type="number" min="0" value="50" class="grow" placeholder="Rewards offerts">\n        <button class="btn-sm cyan" onclick="saveSolv()">Sauver</button>\n      </div>\n      <div id="sm" style="min-height:14px;font-size:11px;font-weight:600"></div>\n    </div>\n\n    <div class="card ai">\n      <div class="ct">◈ EXPLICATION</div>\n      <div style="font-size:12px;line-height:1.8;color:var(--muted)">\n        <p style="margin-bottom:8px">🔴 <b style="color:var(--text)">Si activée</b> — une vente de NXC est bloquée quand les réserves de la banque sont insuffisantes.</p>\n        <p style="margin-bottom:8px">💰 <b style="color:var(--text)">Geste commercial</b> — la banque offre automatiquement X rewards à l\'utilisateur lésé.</p>\n        <p>📋 <b style="color:var(--text)">Tentatives échouées</b> — visibles dans l\'onglet Banque avec bouton "Verser" pour envoyer le geste manuellement.</p>\n      </div>\n    </div>\n  </div>\n\n  <!-- NEXUS APP -->\n  <div class="view" id="view-nexus">\n    <div class="iframe-bar">\n      <span style="color:var(--cyan);font-size:13px;font-weight:800">◈ Nexus Coin</span>\n      <span class="iframe-url">lively-art-86d9.noah-guetta.workers.dev</span>\n      <button class="btn-sm cyan" onclick="$(\'nf\').src=$(\'nf\').src" style="padding:6px 10px;font-size:11px">🔄</button>\n      <a href="https://lively-art-86d9.noah-guetta.workers.dev/nexus_coin_90.html" target="_blank" class="btn-sm gold" style="text-decoration:none;padding:6px 10px;font-size:11px">↗</a>\n    </div>\n    <iframe id="nf" src="https://lively-art-86d9.noah-guetta.workers.dev/nexus_coin_90.html" allow="clipboard-write"></iframe>\n  </div>\n\n</div>\n\n<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>\n<script>\nvar KEY=\'\',mkt={},tInt=null,tMode=null,tStr=0.005,chObj=null,solvOn=false,_users=[];\n\nfunction $(id){return document.getElementById(id);}\nfunction fmt(n,d){return Number(n||0).toLocaleString(\'fr-FR\',{minimumFractionDigits:d||0,maximumFractionDigits:d==null?2:d});}\nfunction esc(s){return (s+\'\').replace(/[&<>"]/g,function(c){return{\'&\':\'&amp;\',\'<\':\'&lt;\',\'>\':\'&gt;\',\'"\':\'&quot;\'}[c];});}\nfunction msg(id,t,ok){var e=$(id);if(!e)return;e.textContent=t;e.style.color=ok?\'var(--green)\':\'var(--red)\';}\n\nasync function api(p,b){\n  b=b||{};b.master_key=KEY;\n  try{var r=await fetch(p,{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify(b)});return await r.json();}\n  catch(e){return{ok:false};}\n}\n\n// ── LOGIN ──\nasync function conn(){\n  KEY=$(\'mk\').value.trim();$(\'lm\').textContent=\'Connexion…\';\n  var r=await api(\'/admin/list\');\n  if(r&&r.ok){\n    $(\'ls\').style.display=\'none\';\n    $(\'hd\').style.background=\'var(--green)\';$(\'hd\').style.boxShadow=\'0 0 8px var(--green)\';\n    $(\'htm\').style.display=\'block\';\n    var s=document.createElement(\'script\');\n    s.src=\'https://cdn.jsdelivr.net/npm/hammerjs@2.0.8/hammer.min.js\';\n    s.onload=function(){var s2=document.createElement(\'script\');s2.src=\'https://cdn.jsdelivr.net/npm/chartjs-plugin-zoom@2.0.1/dist/chartjs-plugin-zoom.min.js\';document.head.appendChild(s2);};\n    document.head.appendChild(s);\n    ref();loadBank();loadSolv();loadFails();\n    setInterval(function(){ref();},15000);\n    setInterval(function(){loadBank();loadFails();},20000);\n    setInterval(function(){$(\'htm\').textContent=new Date().toLocaleTimeString(\'fr-FR\');},1000);\n  }else{$(\'lm\').textContent=\'❌ Clé incorrecte\';}\n}\n\n// ── TABS ──\nfunction go(tab,btn){\n  document.querySelectorAll(\'.view\').forEach(function(v){v.classList.remove(\'on\');});\n  document.querySelectorAll(\'.tab\').forEach(function(t){t.classList.remove(\'on\');});\n  var v=$(\'view-\'+tab);if(v)v.classList.add(\'on\');\n  if(btn)btn.classList.add(\'on\');\n  if(tab===\'users\')loadUsers();\n  if(tab===\'banque\'){$(\'nd-banque\').style.display=\'none\';}\n}\n\n// ── MARCHÉ ──\nvar _prevPrice=0;\nasync function ref(){\n  try{\n    var r=await fetch(\'/nxc/price\');var d=await r.json();mkt=d;\n    var p=parseFloat(d.price||0),h=d.history||[];\n    var chg=_prevPrice>0?((p-_prevPrice)/_prevPrice*100):0;\n    $(\'s-p\').textContent=fmt(p,2);$(\'s-v\').textContent=fmt(d.volume24||0,0);$(\'s-t\').textContent=d.trades24||0;$(\'s-h\').textContent=h.length;\n    $(\'hp\').textContent=fmt(p,2)+\' R\';\n    var hc=$(\'hc\');\n    if(_prevPrice>0){hc.textContent=(chg>=0?\'▲+\':\'▼\')+chg.toFixed(2)+\'%\';hc.className=\'hud-chg \'+(chg>=0?\'up\':\'dn\');}\n    _prevPrice=p;\n    drawC(h);drawA(p,h);\n  }catch(e){}\n}\n\nfunction drawC(h){\n  var cv=$(\'ch\');if(!cv||!window.Chart)return;\n  var pts=h.slice(-100);\n  var labs=pts.map(function(x){return new Date(x.ts).toLocaleTimeString(\'fr-FR\',{hour:\'2-digit\',minute:\'2-digit\'});});\n  var prices=pts.map(function(x){return parseFloat(x.price);});\n  if(prices.length<2)return;\n  var mn=Math.min.apply(null,prices)*0.85,mx=Math.max.apply(null,prices)*1.15;\n  if(chObj){chObj.data.labels=labs;chObj.data.datasets[0].data=prices;chObj.options.scales.y.min=mn;chObj.options.scales.y.max=mx;chObj.update(\'none\');return;}\n  var ctx=cv.getContext(\'2d\');\n  var g=ctx.createLinearGradient(0,0,0,cv.offsetHeight||200);g.addColorStop(0,\'rgba(0,229,255,.22)\');g.addColorStop(1,\'rgba(0,229,255,0)\');\n  chObj=new Chart(ctx,{type:\'line\',data:{labels:labs,datasets:[{data:prices,borderColor:\'#00e5ff\',backgroundColor:g,borderWidth:2.5,pointRadius:0,fill:true,tension:0.4}]},\n    options:{responsive:true,maintainAspectRatio:false,\n      plugins:{legend:{display:false},zoom:{zoom:{wheel:{enabled:true},pinch:{enabled:true},mode:\'x\'},pan:{enabled:true,mode:\'x\'}}},\n      scales:{x:{ticks:{color:\'#5c6b8c\',maxTicksLimit:5,font:{size:8}},grid:{color:\'rgba(0,229,255,.04)\'}},\n        y:{min:mn,max:mx,ticks:{color:\'#5c6b8c\',callback:function(v){return fmt(v,0);}},grid:{color:\'rgba(0,229,255,.04)\'}}},\n      animation:{duration:0}}});\n}\n\nfunction drawA(p,h){\n  var el=$(\'al\'),a=[];\n  if(p>80000)a.push({c:\'aw\',m:\'⚡ Prix très élevé (>80 000 R) — résistance probable\'});\n  else if(p<500)a.push({c:\'ae\',m:\'🔴 Prix très bas (<500 R) — zone critique\'});\n  else a.push({c:\'ao\',m:\'✅ Prix normal (\'+(fmt(p,0))+\' R)\'});\n  if(h.length>10){var rv=h.slice(-10).map(function(x){return x.price;});var vol=(Math.max.apply(null,rv)-Math.min.apply(null,rv))/Math.min.apply(null,rv)*100;a.push(vol>20?{c:\'aw\',m:\'⚡ Forte volatilité: \'+vol.toFixed(1)+\'%\'}:{c:\'ao\',m:\'📊 Marché stable — volatilité: \'+vol.toFixed(1)+\'%\'});}\n  a.push(tMode?{c:\'aw\',m:\'📊 Tendance \'+(tMode===\'up\'?\'📈 Hausse\':tMode===\'down\'?\'📉 Baisse\':\'🎲 Aléatoire\')+\' · \'+(tStr*100).toFixed(1)+\'%/tick\'}:{c:\'ao\',m:\'⏸ Aucune tendance active\'});\n  if(h.length>0){var last=h[h.length-1];a.push({c:\'ai\',m:\'🕒 Dernier tick : \'+new Date(last.ts).toLocaleTimeString(\'fr-FR\')});}\n  el.innerHTML=a.map(function(x){return \'<div class="ab \'+x.c+\'">\'+x.m+\'</div>\';}).join(\'\');\n}\n\nasync function setP(){\n  var p=parseFloat($(\'np\').value);\n  if(!p||p<50||p>100000){msg(\'pm\',\'Prix invalide (50–100 000)\',false);return;}\n  var r=await fetch(\'/nxc/tick\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,price:p,ts:Date.now(),vol:0,volume24:mkt.volume24||0,trades24:mkt.trades24||0})});\n  var res=await r.json();\n  msg(\'pm\',res.ok?\'✅ Cours mis à jour : \'+fmt(p,2)+\' R\':\'❌ Erreur\',res.ok);\n  if(res.ok){$(\'np\').value=\'\';setTimeout(ref,500);}\n}\n\nasync function resetH(){\n  if(!confirm(\'Remettre l\\\'historique à zéro ?\'))return;\n  await fetch(\'/nxc/reset\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY})});\n  ref();\n}\n\nfunction setT(m){\n  var s=parseFloat($(\'ts\').value)||0.005;\n  if(tInt){clearInterval(tInt);tInt=null;}\n  tMode=m===\'stop\'?null:m;tStr=s;\n  var el=$(\'tst\'),ht=$(\'ht\');\n  if(!tMode){el.textContent=\'⏸ Arrêté\';el.style.color=\'var(--muted)\';ht.style.display=\'none\';return;}\n  var lbl=m===\'up\'?\'📈 Hausse +\':m===\'down\'?\'📉 Baisse -\':\'🎲 Aléatoire\';\n  var spd=m!==\'random\'?\' \'+(s*100).toFixed(1)+\'%/tick\':\'\';\n  el.textContent=lbl+spd;el.style.color=m===\'up\'?\'var(--green)\':m===\'down\'?\'var(--red)\':\'var(--purple)\';\n  ht.textContent=lbl+spd;ht.style.display=\'block\';\n  tInt=setInterval(async function(){\n    var p=parseFloat(mkt.price||5213);\n    var adj=(Math.random()-0.48)*0.008;\n    if(m===\'up\')adj+=s;else if(m===\'down\')adj-=s;\n    p=Math.max(50,Math.min(100000,p*(1+adj)));\n    p=Math.random()>0.03?Math.round(p*100)/100:Math.round(p);\n    await fetch(\'/nxc/tick\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,price:p,ts:Date.now(),vol:100,volume24:(mkt.volume24||0)+100,trades24:(mkt.trades24||0)+1})});\n  },12000);\n}\n\n// ── BANQUE ──\nasync function loadBank(){\n  try{\n    var r=await fetch(\'/nxc/bank\');var d=await r.json();if(!d.ok)return;var b=d.bank||{};\n    $(\'bk-r\').textContent=fmt(b.reserves||0,0)+\' R\';$(\'bk-i\').textContent=fmt(b.totalIn||0,0)+\' R\';\n    $(\'bk-o\').textContent=fmt(b.totalOut||0,0)+\' R\';\n    var ratio=b.totalIn>0?((b.reserves||0)/b.totalIn*100):100;$(\'bk-rt\').textContent=ratio.toFixed(1)+\'%\';\n    var fl=$(\'bk-flux\');var flux=(b.flux||[]).slice().reverse().slice(0,25);\n    fl.innerHTML=flux.length?flux.map(function(f){return \'<div class="fl"><span class="\'+(f.type===\'IN\'?\'fl-in\':\'fl-out\')+\'">\'+(f.type===\'IN\'?\'+\':\'-\')+fmt(f.amount||0,0)+\' R</span><span class="fl-user">\'+esc(f.user||\'?\')+\'</span><span class="fl-time">\'+new Date(f.ts).toLocaleTimeString(\'fr-FR\')+\'</span></div>\';}).join(\'\'):\'<p style="color:var(--muted);padding:12px;font-size:12px;text-align:center">Aucun flux</p>\';\n  }catch(e){}\n}\n\nasync function bankInject(){\n  var amt=parseFloat($(\'bk-amt\').value);if(!amt||amt<=0){msg(\'bk-msg\',\'Montant invalide\',false);return;}\n  var cur=await(await fetch(\'/nxc/bank\')).json();var b=cur.bank||{reserves:0,totalIn:0,totalOut:0,nxcEmis:0,flux:[]};\n  b.reserves=parseFloat(((b.reserves||0)+amt).toFixed(2));b.totalIn=parseFloat(((b.totalIn||0)+amt).toFixed(2));\n  b.flux=b.flux||[];b.flux.push({type:\'IN\',user:\'SERVEUR\',amount:amt,nxc:0,ts:Date.now()});\n  var r=await fetch(\'/nxc/bank\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,bank:b,reset:true})});\n  var res=await r.json();msg(\'bk-msg\',res.ok?\'+\'+fmt(amt,0)+\' R injectés ✅\':\'❌ Erreur\',res.ok);\n  $(\'bk-amt\').value=\'\';if(res.ok)loadBank();\n}\n\nasync function bankRetire(){\n  var amt=parseFloat($(\'bk-amt\').value);if(!amt||amt<=0){msg(\'bk-msg\',\'Montant invalide\',false);return;}\n  var cur=await(await fetch(\'/nxc/bank\')).json();var b=cur.bank||{reserves:0,totalIn:0,totalOut:0,nxcEmis:0,flux:[]};\n  if(amt>(b.reserves||0)){msg(\'bk-msg\',\'Réserves insuffisantes ❌\',false);return;}\n  b.reserves=parseFloat(((b.reserves||0)-amt).toFixed(2));b.totalOut=parseFloat(((b.totalOut||0)+amt).toFixed(2));\n  b.flux=b.flux||[];b.flux.push({type:\'OUT\',user:\'SERVEUR\',amount:amt,nxc:0,ts:Date.now()});\n  var r=await fetch(\'/nxc/bank\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,bank:b,reset:true})});\n  var res=await r.json();msg(\'bk-msg\',res.ok?\'-\'+fmt(amt,0)+\' R retirés ✅\':\'❌ Erreur\',res.ok);\n  $(\'bk-amt\').value=\'\';if(res.ok)loadBank();\n}\n\nasync function bankResetHist(){\n  var cur=await(await fetch(\'/nxc/bank\')).json();var b=cur.bank||{};var g=b.reserves||0;\n  if(!confirm(\'Effacer historique ?\\nRéserves conservées: \'+fmt(g,0)+\' R\'))return;\n  var r=await fetch(\'/nxc/bank\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,bank:{reserves:g,nxcEmis:0,totalIn:0,totalOut:0,flux:[]},reset:true})});\n  var res=await r.json();msg(\'bk-msg\',res.ok?\'✅ Historique effacé\':\'❌ Erreur\',res.ok);if(res.ok)loadBank();\n}\n\nasync function bankResetAll(){\n  var cur=await(await fetch(\'/nxc/bank\')).json();var b=cur.bank||{};\n  var g=confirm(\'Garder les réserves (\'+fmt(b.reserves||0,0)+\' R) ?\\nOK = garder | Annuler = tout à zéro\');\n  if(!confirm(\'Confirmer la réinitialisation ?\'))return;\n  var r=await fetch(\'/nxc/bank\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,bank:{reserves:g?(b.reserves||0):0,nxcEmis:0,totalIn:0,totalOut:0,flux:[]},reset:true})});\n  var res=await r.json();msg(\'bk-msg\',res.ok?\'✅ Réinitialisé\':\'❌ Erreur\',res.ok);if(res.ok)loadBank();\n}\n\nasync function loadFails(){\n  try{\n    var r=await fetch(\'/nxc/bank/fail\');var d=await r.json();\n    var el=$(\'bk-fails\');if(!el)return;\n    var fails=(d.fails||[]).slice().reverse();\n    var dot=$(\'nd-banque\');\n    if(fails.length&&dot)dot.style.display=\'block\';\n    if(!fails.length){el.innerHTML=\'<p style="color:var(--muted);padding:12px;font-size:12px;text-align:center">Aucune tentative échouée ✅</p>\';return;}\n    el.innerHTML=fails.map(function(f){\n      return \'<div class="fail"><div class="fail-top"><span style="color:var(--red);font-weight:700">❌ \'+esc(f.user)+\'</span><span style="color:var(--muted);font-size:10px">\'+new Date(f.ts).toLocaleTimeString(\'fr-FR\')+\'</span></div>\'\n        +\'<div style="color:var(--muted);font-size:11px">Voulait vendre \'+f.nxc+\' NXC (\'+fmt(f.amount||0,0)+\' R)</div>\'\n        +(f.gesture>0?\'<button onclick="sendGesture(\\\'\'+esc(f.user)+\'\\\',\'+f.gesture+\',\'+f.ts+\')" style="padding:7px 14px;background:rgba(0,255,157,.1);border:1px solid rgba(0,255,157,.3);border-radius:8px;color:var(--green);font-size:12px;cursor:pointer;font-weight:700;align-self:flex-start">💝 Verser +\'+f.gesture+\' R</button>\':\'\')\n        +\'</div>\';\n    }).join(\'\');\n  }catch(e){}\n}\n\nasync function sendGesture(user,amount,failTs){\n  if(!confirm(\'Verser \'+amount+\' R à \'+user+\' ?\'))return;\n  var r=await fetch(\'/nxc/bank/gesture\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,target:user,amount:amount,fail_ts:failTs})});\n  var res=await r.json();\n  msg(\'bk-msg\',res.ok?\'✅ \'+amount+\' R versés à \'+user:\'❌ Erreur: \'+(res.error||\'?\'),res.ok);\n  loadBank();loadFails();\n}\n\n// ── UTILISATEURS ──\nasync function loadUsers(){\n  $(\'us-msg\').textContent=\'Chargement…\';\n  try{\n    var r=await api(\'/admin/list\');if(!r||!r.ok){$(\'us-msg\').textContent=\'Erreur\';return;}\n    var p=parseFloat(mkt.price||0);\n    var rows=await Promise.all((r.users||[]).map(async function(u){\n      var d=await api(\'/admin/get\',{target:u.username});\n      var rew=Math.max((d.data&&d.data.nx2098&&d.data.nx2098.rewards)||0,(d.data&&d.data.rewards&&d.data.rewards.points)||0);\n      var nxc=parseFloat((d.data&&d.data.nxcoin&&d.data.nxcoin.nxc)||0);\n      return {n:u.username,role:u.role,rew:rew,nxc:nxc,val:nxc*p};\n    }));\n    _users=rows;renderU(rows);$(\'us-msg\').textContent=\'\';\n  }catch(e){$(\'us-msg\').textContent=\'Erreur de chargement\';}\n}\nfunction renderU(rows){\n  $(\'ut\').innerHTML=rows.map(function(r){\n    return \'<tr><td style="font-weight:700;color:var(--cyan)">\'+esc(r.n)+(r.role===\'admin\'?\' 👑\':\'\')+\'</td>\'\n      +\'<td style="color:var(--muted);font-size:11px">\'+esc(r.role)+\'</td>\'\n      +\'<td style="color:var(--gold)">\'+fmt(r.rew,0)+\'</td>\'\n      +\'<td style="color:var(--cyan);font-family:monospace">\'+r.nxc.toFixed(4)+\'</td>\'\n      +\'<td style="color:var(--purple)">\'+fmt(r.val,0)+\'</td></tr>\';\n  }).join(\'\');\n}\nfunction filterU(){\n  var q=($(\'us-q\').value||\'\').toLowerCase();\n  renderU(q?_users.filter(function(r){return r.n.toLowerCase().includes(q);}):_users);\n}\n\n// ── SOLVABILITÉ ──\nasync function loadSolv(){\n  try{\n    var r=await fetch(\'/nxc/solvability\');var d=await r.json();\n    if(d.ok){solvOn=d.enabled;var inp=$(\'sg\');if(inp)inp.value=d.gesture||50;updSolv();}\n  }catch(e){}\n}\nfunction updSolv(){\n  var t=$(\'stg\'),l=$(\'sl\');\n  if(solvOn){if(t)t.classList.add(\'on\');if(l){l.textContent=\'✅ Activée\';l.style.color=\'var(--green)\';}}\n  else{if(t)t.classList.remove(\'on\');if(l){l.textContent=\'⏸ Désactivée\';l.style.color=\'var(--muted)\';}}\n}\nasync function toggleSolv(){solvOn=!solvOn;updSolv();await saveSolv();}\nasync function saveSolv(){\n  var g=parseInt($(\'sg\').value)||50;\n  var r=await fetch(\'/nxc/solvability\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({master_key:KEY,enabled:solvOn,gesture:g})});\n  var res=await r.json();\n  msg(\'sm\',res.ok?(solvOn?\'✅ Solvabilité activée\':\'⏸ Solvabilité désactivée\'):\'❌ Erreur\',res.ok);\n}\n</script>\n</body>\n</html>\n'
